...
task SubsetVcfBySamplesList {
  ...
  output {
    File vcf_subset_by_sample = vcf_subset_filename
    File vcf_subset_by_sample_index = vcf_subset_idx_filename
  }
...
#subset a VCF to a specific contig
...
#concatinate vcfs across contigs
task ConcatVcfs {
  input {
    Array[File] vcfs
    Array[File]? vcfs_idx
