...
workflow AnnotateVcfWithRandomForestScores {
  input {
    File vcf
    File vcf_idx
  ...
  call MergeVcfs {
    input:
      vcfs=AnnotateRFScores.annotated_vcf,
      vcf_idxs=AnnotateRFScores.annotated_vcf_idx,
...
task SubsetVcf {
  ...
  command <<<
    ...
    find boost_results -name "RF_results.*.tsv" \
    | xargs -I {} mv {} boost_results/

    # Get list of sample IDs
    find boost_results/ -name "*.tsv" \
    | xargs -I {} basename {} \
    | sed 's/\./\t/g' | cut -f2 \
    | sort -Vk1,1 | uniq \
    > samples.list
...
task AnnotateRFScores {
  ...
  command <<<
    ...
    while read sample; do
      infile=$( find boost_results -name "RF_results.$sample.tsv" | sed -n '1p' )
      if ! [ -z $infile ]; then
        awk -F'\t' -v OFS='\t' 'ARGIND==1{inFileA[$1]; next} {if ($1 in inFileA) print }' vids.list $infile > boost_results/$sample.scores.tsv
        echo -e "$sample\tboost_results/$sample.scores.tsv" >> boost_anno_inputs.tsv
      fi
    done < samples.list

    # Add Boost scores to VCF
    /src/add_random_forest_scores_to_vcf.py \
      --vcf ~{vcf} \
      --RF_tsv boost_anno_inputs.tsv \
...
task MergeVcfs {
  ...
  RuntimeAttr runtime_default = object {
      mem_gb: 2,
      disk_gb: ceil(base_disk_gb + (input_size * 4.0)),
      cpu_cores: 1,
      preemptible_tries: 0,
      max_retries: 1,
      boot_disk_gb: 10
