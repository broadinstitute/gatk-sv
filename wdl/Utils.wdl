...
task RenameVcfSamples {
  input {
    File vcf
    File vcf_idx
    File sample_renaming_json  # JSON with map from old sample IDs to new sample IDs
    String samtools_cloud_docker
  ...
  String vcf_renamed_samples_filename = basename(vcf, ".vcf.gz") + "_~{renamed_suffix}.vcf.gz"
  String vcf_renamed_samples_index_filename = vcf_renamed_samples_filename + ".tbi"
  ...
  command <<<
    ...
    echo "Get original VCF samples"
    export GCS_OAUTH_TOKEN=`gcloud auth application-default print-access-token`
    bcftools query -l ~{vcf} > original_vcf_samples.list

    echo "rename samples according to JSON map, getting the new corresponding list of sample IDs"
    python - <<____EoF
import json
with open("~{sample_renaming_json}", 'r') as f_in:
  sample_ids_map = json.load(f_in)

with open("original_vcf_samples.list", 'r') as f_in, open("renamed_samples.list", 'w') as f_out:
  for line in f_in:
    sample_id = line.rstrip()
    renamed_id = sample_ids_map.get(sample_id, sample_id)
    f_out.write(renamed_id + "\n")
____EoF

    echo "use bcftools reheader to change the sample names"
    bcftools reheader \
      -s "renamed_samples.list" \
      -o ~{vcf_renamed_samples_filename} \
      ~{vcf}

    echo "index the new vcf"
    bcftools index --force --tbi --threads 2 ~{vcf_renamed_samples_filename}
  >>>

  output {
    File vcf_renamed = vcf_renamed_samples_filename
    File vcf_renamed_idx = vcf_renamed_samples_index_filename
  }
