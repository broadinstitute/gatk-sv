...
task LocalizeCloudFileAndIndexWithCredentials {
  input {
    String cloud_file_path
    String cloud_file_index_path
    String service_account_json
    Int disk_size
    String cloud_sdk_docker
    RuntimeAttr? runtime_attr_override
  ...
  command {
    ...
    gsutil cp '~{cloud_file_path}' .
    gsutil cp '~{cloud_file_index_path}' .
  }

  output {
    File output_file = basename(cloud_file_path)
    File output_file_index = basename(cloud_file_index_path)
  }
