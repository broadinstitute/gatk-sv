...
workflow ResolveComplexVariants {
  input {
    ...
    File cluster_vcf
    File cluster_bothside_pass_list
    File cluster_background_fail_list
  ...
  call BreakpointOverlap {
    input:
      ...
      bothside_pass_list=cluster_bothside_pass_list,
      background_fail_list=cluster_background_fail_list,
      sv_pipeline_docker=sv_pipeline_docker,
  ...
  call MiniTasks.UpdateSrList as UpdateBothsidePass {
    input:
      ...
      original_list=cluster_bothside_pass_list,
  ...
  call MiniTasks.UpdateSrList as UpdateBackgroundFail {
    input:
      vcf=RenameVariants.renamed_vcf,
      original_list=cluster_background_fail_list,
  ...
  #Final outputs
  output {
    File complex_resolve_vcf = RenameVariants.renamed_vcf
    File complex_resolve_vcf_index = RenameVariants.renamed_vcf_index
    File complex_resolve_bothside_pass_list = UpdateBothsidePass.updated_list
    File complex_resolve_background_fail_list = UpdateBackgroundFail.updated_list
    File breakpoint_overlap_dropped_record_vcf = BreakpointOverlap.dropped_record_vcf
    File breakpoint_overlap_dropped_record_vcf_index = BreakpointOverlap.dropped_record_vcf_index
  }
}
