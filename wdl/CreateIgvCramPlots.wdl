...
workflow IgvAllSamples {
    ...
    scatter (family_name in GetSamples.family_list) {
        ...
        if(file_localization){
            call igv_plots.Igv as IgvLocalize {
                input:
                    family = family_name,
                    samples = GetFamilySample.family_sample_list,
                    cram_crai_list = GetSampleCramCrai.sample_crai_cram_list,
                    varfile = varfile,
        ...
        if(!file_localization){
            call igv_plots.Igv as IgvParse {
                input:
                    family = family_name,
                    samples = GetFamilySample.family_sample_list,
                    cram_crai_list = GetSampleCramCrai.sample_crai_cram_list,
    ...
    call IntegrateIgvPlots{
        input:
            igv_tar = select_all(flatten([IgvLocalize.tar_gz_pe, IgvParse.tar_gz_pe])),
            prefix = prefix,
            variant_interpretation_docker = variant_interpretation_docker,
            runtime_attr_override = runtime_attr_run_igv
    }
...
task GetSampleCramCrai {
    ...
    Float input_size = size([ped_file, sample_crai_cram], "GB")
    Float base_mem_gb = 3.75
    ...
    output {
        File sample_crai_cram_list = "sample_crai_cram_list.txt"
    }
    ...
    command <<<
        ...
        # Filter CRAM/CRAI list to only include samples in ped file
        head -n 1 ~{sample_crai_cram} > sample_crai_cram_list.txt
        while read sample; do
            grep "^$sample" ~{sample_crai_cram} >> sample_crai_cram_list.txt || true
