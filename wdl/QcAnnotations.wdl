...
task PlotSiteLevelBenchmarking {
    ...
    command <<<  
        ...
        tar -czvf \
            ~{tarball_dir_name}.wPlots.tar.gz \
            ~{tarball_dir_name}
    >>>
...
task PlotQcPerFamily {
    ...
    command <<<
        ...
        tar -czvf \
            ~{prefix}.plotQC_perFamily.tar.gz \
            ~{prefix}_perFamily_plots
    >>>
...
task PlotPerSampleBenchmarking {
    ...
    command <<<
        ...
        tar -xvzf \
            ~{per_sample_benchmarking_tarball} \
            --directory tmp_untar/
        ...
        tar -czvf \
            ~{prefix}.~{comparison_set_name}_perSample_benchmarking_plots.tar.gz \
            ~{prefix}.~{comparison_set_name}_perSample_benchmarking_plots
    >>>
...
task SanitizeOutputs {
    ...
    command <<<
        ...
        if ~{defined(cleaned_fam_file)}; then
            cp ~{cleaned_fam_file} ~{prefix}_SV_VCF_QC_output/data/~{prefix}.cleaned_trios.fam || true
        fi

        cp ~{samples_list} ~{prefix}_SV_VCF_QC_output/data/~{prefix}.samples_analyzed.list

        tar -czvf \
            ~{prefix}_SV_VCF_QC_output.tar.gz \
