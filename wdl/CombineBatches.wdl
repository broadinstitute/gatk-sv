...
import "ExtraScatter.wdl" as extra
...
workflow CombineBatches {
  ...
  scatter ( i in range(n_shards) ) {
    ...
    call extra.ExtraScatter {
      input:
        vcf_tarballs=ScatterVcf.shards_tarball,
        initial_shard_number = i,
        ploidy_table=CreatePloidyTableFromPed.out,
        batches=batches,
        records_per_shard_extra_join=records_per_shard_extra_join,
        prefix="~{cohort_name}.combine_batches.~{contig}.join_vcfs",
        contig=contig,
        ...
        sv_pipeline_docker=sv_pipeline_docker,
        sv_base_mini_docker=sv_base_mini_docker,
        runtime_attr_extra_scatter=runtime_attr_extra_scatter,
        runtime_attr_extra_join=runtime_attr_extra_join,
        runtime_attr_extra_concat=runtime_attr_extra_concat,
  ...
  call MiniTasks.ConcatVcfs as ConcatJoinVcfs {
    input:
      vcfs=ExtraScatter.joined_shard,
      vcfs_idx=ExtraScatter.joined_shard_index,
      naive=true,
