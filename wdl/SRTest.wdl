...
workflow SRTest {
  ...
  call tasksbatchmetrics.MergeStats as MergeStats {
    input:
      stats = select_all(flatten([SRTestAutosome.stats, SRTestAllosome.stats])),
      prefix = "${batch}.${algorithm}",
      linux_docker = linux_docker,
      runtime_attr_override = runtime_attr_merge_stats
  }

  Array[File] common_stats = select_all(SRTestAutosome.stats_common)
  if (run_common && (length(common_stats) > 0)) {
