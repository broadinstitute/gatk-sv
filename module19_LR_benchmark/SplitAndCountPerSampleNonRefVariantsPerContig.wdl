...
workflow SplitAndCountPerSampleNonRefVariantsPerContig {
  input {
    File input_vcf
    String? contig_label
  ...
  scatter (sample_name in effective_samples) {
    ...
    call CountSampleVariantCategories {
      input:
        sample_name            = sample_name,
        contig_label           = contig,
  ...
  call MergeSampleCountTables {
    input:
      sample_count_tables    = CountSampleVariantCategories.count_tsv,
      merged_output          = output_prefix + "." + contig + ".sample_category_counts.tsv",
      python_docker          = python_docker,
  ...
  output {
    ...
    Array[File] per_sample_count_tables = CountSampleVariantCategories.count_tsv
    Array[Array[File]] per_sample_category_vcfs = CountSampleVariantCategories.category_vcfs
...
task CountSampleVariantCategories {
  input {
    String sample_name
    String contig_label
