...
* [Module 04](#module04) is run separately per batch, using filtered variants ([FilterBatch](#filter-batch) output) combined across all batches
...
Trains a gCNV model for use in [GatherBatchEvidence](#gather-batch-evidence). The WDL can be found at `/wdl/TrainGCNV.wdl`.
