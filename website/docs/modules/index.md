...
implemented as a separate runnable workflow.
...
pre-configured with default values for all required parameters and configured to run the pipeline for most use cases. 

The following sections supplement the Terra workspaces with documentation for each WDL, including an overview of its 
function, key input parameters, and outputs. Not all parameters are included here, as some WDLs contain dozens of 
inputs. Users are encouraged to refer to the WDL source code for additional clarification.

For details on executing joint calling in Terra, refer to the [Workflow instructions](/docs/execution/joint#instructions) 
section.
