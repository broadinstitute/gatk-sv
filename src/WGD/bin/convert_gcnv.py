...
def write_bed_entry(file, record, score, type, sample):
  bed_entries = (record.chrom, str(record.start), str(record.stop), str(score), sample, type, 'gcnv')
...
# Main function
def main():
  parser = argparse.ArgumentParser(description="Converts gCNV segments vcf to .del.bed and .dup.bed files")
  parser.add_argument("--input_sample", help="sample id in the vcf (defaults to first sample in vcf header)")
  parser.add_argument("--cutoff", help="QS score cutoff")
  parser.add_argument("ploidy_calls", help="gCNV ploidy calls file (contig_ploidy.tsv)")
  parser.add_argument("segments_vcf", help="gCNV segments vcf")
  parser.add_argument("output_sample", help="sample id to use in output and base name for output files")
  ...
  with open(del_name, 'w') as del_bed, open(dup_name, 'w') as dup_bed:
    ...
    for rec in vcf.fetch():
      if rec.chrom in contig_ploidy:
        ...
        sys.stderr.write("Warning: no ploidy call found for contig: \"" + rec.chrom + "\", record omitted.\n")
