...
# Script to correct for multiple sequence features across a list of binCov files
# Note: Designed to be run on 24 chromosome-split files from the same library

#Requires tab-delimmed input file with the following 4 columns:
# 1: contig name
# 2: full paths to binCov files for each contig
...
readBinCovList <- function(paths,labels=c(1:22,"X","Y"),
  ...
  if(norm==T){
    #Count the number of extra columns
    norm.cols <- ncol(dat[[1]])-3
    if(norm.cols<1){
      stop("Input binCov dataset has fewer than four columns")
    }
    #Apply over each extra columns
    sapply(4:norm.cols,function(c){
      #Get median value across all contigs
      median.all <- median(unlist(sapply(which(!(names(dat) %in% exclude)),function(i){
        return(dat[[i]][,c])
      })),na.rm=T)
      #Normalize all columns by median.all
      for(i in 1:length(dat)){
        dat[[i]][,c] <- dat[[i]][,c]/median.all
      }
    })
  }
...
#Run feature correction per feature column, smooth, and round to nearest whole integer for all autosomes
cov.dat.adj <- cov.dat
for(f in 1:ncol(feature.dat[[1]])){
  cov.dat.adj[1:length(cov.dat.adj)] <- lapply(1:length(cov.dat.adj),function(i){
    #Compute coverage residuals
    #Allosomes: scale to mean coverage of that contig
    #Otherwise: scale to library-wide autosomal mean
    if(names(cov.dat.adj)[i] %in% c("X","Y")){
    ...
    #Adjust coverage
    vals.adj <- cov.dat.adj[[i]]+vals.residuals.smoothed
