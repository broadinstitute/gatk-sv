...
class Keys:
    ...
    scores_file = "scores_parquet_tar"
...
class TruthData:
    ...
    def __init__(
        ...
        self.confident_variants = TruthData.load_confident_variants(truth_json)
...
class ScoresSource:
    """Holds file origin of scores data, as well as DaskDataFrames with scores info

    Attributes:
        scores_parquet_tar: Path to parquet folder with scores data
        score_property: name of string defining the property
        passing_score: nominal minimum score to not be filtered by this source, if any
        wanted_variant_ids: set of IDs to admit when loading / looking at score data
        wanted_sample_ids: sample IDs (in order) to admit when loading / looking at score data
        _scores: dask DataFrame with score data
            - indexed by row
            - one column for variant ID
            - one column for each sample ID, with integer score or pandas.NA
    """
    scores_parquet_tar: Path
    score_property: str = None
    passing_score: Optional[float] = None
    wanted_variant_ids: Optional[set[str]] = None
    wanted_sample_ids: Optional[list[str]] = None
    remove_input_tar: bool = Default.remove_input_tar
    _allele_row_map: Optional[DaskSeries] = None
    _variant_ids: Optional[DaskSeries] = None
    _scores: Optional[DaskDataFrame] = None
    ...
    def _load_scores_and_ids(self):
        # always load scores and IDs together to keep the divisions/partitions in sync
        self._variant_ids = _load_parquet_file_properties(
            parquet_path=self.scores_parquet_tar,
            wanted_properties=[Keys.id],
            wanted_variant_ids=self.wanted_variant_ids,
            wanted_sample_ids=self.wanted_sample_ids,
        )[Keys.id]
        self._scores = _load_parquet_file_properties(
            parquet_path=self.scores_parquet_tar,
    ...
    @property
    def scores(self) -> DaskDataFrame:
        if self._scores is None:
            self._load_scores_and_ids()
        return self._scores

    @property
    def variant_ids(self) -> pandas.Series:
        if self._variant_ids is None:
            # always load scores and IDs together to keep the divisions/partitions in sync
            self._load_scores_and_ids()
        return self._variant_ids

    def _get_first_score_column(self) -> tuple[Optional[str], str]:
        first_score_column = next(
            (
                _c for _c in tarred_properties_to_parquet.get_parquet_folder_unflat_columns(
                    self.scores_parquet_tar,
                    remove_input_tar=self.remove_input_tar
                )
                if _c[1] == self.score_property
            ),
            None
        )
        if first_score_column is None:
            raise ValueError(
                f"Unable to find score property {self.score_property} in {self.scores_parquet_tar}"
            )
        return first_score_column

    @property
    def is_variant_filter(self) -> bool:
        return self._get_first_score_column()[0] is None

    @property
    def is_genotype_filter(self) -> bool:
        return self._get_first_score_column()[0] is not None

    def _updated_variant_ids(
        self, new_available_variant_ids: Optional[set[str]]
    ) -> Optional[set[str]]:
        if self.wanted_variant_ids is None:
            return new_available_variant_ids
        elif new_available_variant_ids is not None:
            return self.wanted_variant_ids.intersection(new_available_variant_ids)
        else:
            return self.wanted_variant_ids

    def filter_variants(self, wanted_allele_variants: pandas.Series) -> "ScoresSource":
        row_is_wanted = self.variant_ids.isin(wanted_allele_variants)

        return ScoresSource(
            scores_parquet_tar=self.scores_parquet_tar,
            score_property=self.score_property,
            passing_score=self.passing_score,
            wanted_variant_ids=None,
            wanted_sample_ids=self.wanted_sample_ids,
            remove_input_tar=self.remove_input_tar,
            variant_ids=self._variant_ids[row_is_wanted],
            scores=self._scores.loc[row_is_wanted],
        )
...
class ScoresDataSet:
    ...
    Attributes:
        variants_parquet: Path to parquet folder / tarfile with metrics and allele counts
        scores_sources: dict from source label to ScoresSource objects with genotype / variant
                        scores. Since filtering will involve updating allele counts and scores in
                        tandem, eventually the ScoresDataSet must be split into multiple
                        ScoresDataSets with only one ScoresSource each.
        label: name of the data set
        category: name of category of SVs that are being subset to
        wanted_variant_ids: set of IDs to admit when loading / looking at metric / allele data
        wanted_sample_ids: sample IDs (in order) to admit when loading / looking at data
        _variant_ids: Series with variant IDs
        _allele_counts: DataFrame with columns = sample ID and rows corresponding to _variant_ids.
                        Entries are allele count: -1=no-call, 0=ref, 1=het, 2=homvar
        _metrics: DataFrame with columns = Keys.svtype, Keys.svlen, Keys.is_autosome and rows
    ...
    wanted_sample_ids: Optional[list[str]] = None
    remove_input_tar: bool = Default.remove_input_tar
    _variant_ids: Optional[pandas.Series] = None
    _allele_counts: Optional[DaskDataFrame] = None
    _metrics: Optional[DaskDataFrame] = None
    ...
    def _load_all_properties(self):
        """Load all properties simultaneously to keep divisions/partitions in sync"""
        # unfortunately Dask has performance problems comparing the variant IDs in ScoresDataSet
        # and ScoresSource if they are both DaskSeries. compute the ScoresDataSet variant IDs to
        # convert them to a pandas.Series, and the performance improves a lot. The amount of memory
        # required is fairly low: need to store a Series of strings that is num_variants long, so
        # it should be in the realm of MB, not GB.
        self._variant_ids = _load_parquet_file_properties(
            parquet_path=self.variants_parquet,
            wanted_properties=[Keys.id],
            wanted_variant_ids=self.wanted_variant_ids,
            wanted_sample_ids=self.wanted_sample_ids,
            remove_input_tar=self.remove_input_tar
        )[Keys.id].compute()
        self._metrics = _load_parquet_file_properties(
            parquet_path=self.variants_parquet,
        ...
        self._allele_counts = _load_parquet_file_properties(
            parquet_path=self.variants_parquet,
    ...
    def scores_set_label(self) -> str:
        return f"{self.label}, {next(iter(self.scores_sources.keys()))}"

    @property
    def metrics(self) -> DaskDataFrame:
    ...
    @property
    def allele_counts(self) -> DaskDataFrame:
        if self._allele_counts is None:
            self._load_all_properties()
        return self._allele_counts

    @staticmethod
    def from_json(
            json_file: Path, temp_dir: Path, remove_input_tar: bool
    ) -> list["ScoresDataSet"]:
        with open(json_file, 'r') as f_in:
            all_data_sets_info = json.load(f_in)
        return [
            ScoresDataSet(
                variants_parquet=Path(data_set_info[Keys.variants_parquet]),
                scores_sources={
                    source_label: ScoresSource(
                        scores_parquet_tar=Path(
                            source_info.get("scores_parquet_tar",
                                            data_set_info[Keys.variants_parquet])
                        ),
                        remove_input_tar=remove_input_tar,
                        **{key: val for key, val in source_info.items()
                           if key != "scores_parquet_tar"}
                    )
    ...
    def filter_variants(
        ...
        filtered_variant_ids = self.variant_ids[row_is_wanted]
        return ScoresDataSet(
            variants_parquet=self.variants_parquet,
            scores_sources={
                source_label: scores_source.filter_variants(filtered_variant_ids)
            ...
            wanted_variant_ids=None,
            wanted_sample_ids=self.wanted_sample_ids,
            variant_ids=filtered_variant_ids,
            metrics=None if self._metrics is None else self._metrics.loc[row_is_wanted, :],
            allele_counts=(
                None if self._allele_counts is None else self._allele_counts.loc[row_is_wanted, :]
            ),
            inheritance_stats=(
                None if self._inheritance_stats is None
                else self._inheritance_stats.loc[row_is_wanted.compute(), :]
            ),
    ...
    def num_variants(self) -> int:
        return dask_utils.compute_if_unknown(self.variant_ids.shape[0])
    ...
    def split_scores_sources(self) -> list["ScoresDataSet"]:
        """Split the scores sources so that that allele_counts can be modified """
        if len(self.scores_sources) == 1:
            return [self]
        else:
            return [
                ScoresDataSet(
                    ...
                    allele_counts=self._allele_counts,
                    metrics=self._metrics,
                    inheritance_stats=self._inheritance_stats,
                    num_called_genotypes=self._num_called_genotypes
                )
                for source_label, scores_source in self.scores_sources.items()
            ]

    @property
    def scores_source(self) -> ScoresSource:
        if len(self.scores_sources) > 1:
            raise RuntimeError(
                "apply_hard_filter() can only be invoked on individual ScoresDataSets created by "
                "split_scores_sources()"
            )
        return next(iter(self.scores_sources.values()))

    @property
    def passing_score(self) -> Optional[float]:
        return self.scores_source.passing_score

    def _score_row_is_wanted_to_allele_row_is_wanted(
        self, score_row_is_wanted: DaskSeries
    ) -> DaskSeries:
        """In general, score are obtained from a separate file as the metrics and allele counts,
        and may not have identical variant ID order (some score sources will omit failing rows).
        This function maps between them
        """
        # return self.variant_ids.isin(
        #     set(self.scores_source.variant_ids.loc[score_row_is_wanted].compute())
        # )
        wanted_ids = set(self.scores_source.variant_ids.loc[score_row_is_wanted].compute())
        return self.variant_ids.map_partitions(
            lambda _ids_partition: _ids_partition.isin(wanted_ids),
    ...
    def apply_hard_filter(self, passing_score: Optional[float] = None) -> "ScoresDataSet":
        ...
        """
        _scores_source = self.scores_source
        if passing_score is None:
            # get default passing score from ScoresSource
            passing_score = _scores_source.passing_score
        # noinspection PyTypeChecker
        if _scores_source.is_variant_filter:
            # noinspection PyTypeChecker,PyUnresolvedReferences
            col = _scores_source.scores.columns[0]
            return self.filter_variants(
                row_is_wanted=self._score_row_is_wanted_to_allele_row_is_wanted(
                    _scores_source.scores[col] >= passing_score
                )
            )
            ...
            allele_counts: DaskDataFrame = self.allele_counts.mask(
                _scores_source.scores < passing_score,
                -1
            )

            return ScoresDataSet(
                variants_parquet=self.variants_parquet, scores_sources=self.scores_sources,
                label=self.label, variant_ids=self._variant_ids, allele_counts=allele_counts,
                metrics=self.metrics, category=self.category, inheritance_stats=None,
        ...
        return dask.array.concatenate(
            seq=[
                dask.array.from_delayed(
                    ...
                    shape=(_partition.shape[0].compute(), num_trios, 3),
                    dtype=numpy_dtype,
                )
                for _partition in samples_df.partitions
            ],
            axis=0
        ).compute_chunk_sizes()
    ...
    def set_inheritance_stats(
        ...
        self._inheritance_stats = dask.dataframe.concat(
            [
    ...
    def get_mendelian_violation_curve(
        ...
        if truth_data.pedigree_file_info is None or self.num_variants == 0:
            return None  # no data
        column_infos = [
            dask_utils.get_dtype_info(dt) for dt in set(self.scores_source.scores.dtypes)
        ]
        ...
        min_scores = ScoresDataSet._dataframe_to_trio_3d_dask(
            truth_data=truth_data, samples_df=self.scores_source.scores, numpy_dtype=min_type
        ).min(axis=2)

        # save this here so that temp persisting lasts until after the curve is computed
        trio_categories = self._get_trio_categories(truth_data)
        # compute the scores
    ...
    def get_precision_recall_curve(self, truth_data: TruthData) -> PrecisionRecallCurve:
        ...
        good_value_counts, bad_value_counts = truth_data.to_truth_value_counts(
            values=self.scores_source.scores, ids=self.scores_source.variant_ids
        )
        log("\tgot good/bad value_counts")
        return PrecisionRecallCurve.from_good_bad_value_counts(
            good_value_counts=good_value_counts, bad_value_counts=bad_value_counts
        )
        ...
        for filter_label, filter_score in {
            ...
            for category, selector in iter_categories(sv_selectors):
                ...
                inheritance_stats_dict[(data_set.scores_set_label, filter_label, category)] = (
                    selected_filtered_data_set.inheritance_stats.sum(axis=0)
                )
            ...
            category: pandas.DataFrame.from_dict(
                {
                    ...
                    if (data_set.scores_set_label, filter_label, category) in inheritance_stats_dict
                },
                orient="index"
            )
...
def get_mendelian_violation_curves(
    ...
    mendelian_violation_curves_dict = {
        category: {} for category, __ in iter_categories(sv_selectors)
    }
    for data_set in scores_data_sets:
...
def get_scores_histogram_figure(
    ...
    _draw_extra_legend_axis(
        ...
        line_labels=[scores_data_set.detailed_label for scores_data_set in scores_data_sets],
...
def plot_scores_histogram(
    ...
    for data_set_index, scores_data_set in enumerate(scores_data_sets):
        counts, edges = _get_scores_histogram(
            scores=scores_data_set.select_category(sv_category, sv_selector).scores_source.scores,
        ...
        if counts.size > 0:
            ...
            plotting.barh(ax, values=values, edges=edges, x_baseline=x_baseline,
                          color=data_colors[scores_data_set.detailed_label], fill=True)
...
def get_hardy_weinberg_figure(
    ...
    for scores_data_set in scores_data_sets:
        ...
        plot_hardy_weinberg_proportions(
            ...
            title=scores_data_set.label, title_font_size=title_font_size,
            label_font_size=label_font_size, tick_font_size=tick_font_size,
            min_num_called_gt_hardy_weinberg=min_num_called_gt_hardy_weinberg,
            hardy_weinberg_confidence=hardy_weinberg_confidence
        )
...
def get_filter_quality_figures(
    ...
    scores_data_sets = [
        split_data_set
        for original_scores_data_set in scores_data_sets
        for split_data_set in original_scores_data_set.split_scores_sources()
    ]
...
def load_quality_data(
    ...
    scores_data_sets = ScoresDataSet.from_json(
        scores_data_json, temp_dir=temp_dir, remove_input_tar=remove_input_tar
    )
    all_sv_types = set.union(*(data_set.all_sv_types for data_set in scores_data_sets))
    sv_selectors = get_truth_overlap.get_sv_selectors(
...
def __parse_arguments(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="path to json file with object mapping from data set label to object with keys:\n"
             ...
             f"{Keys.variants_parquet}: array of paths to files with variant metrics\n"
...
def main(argv: Optional[list[str]] = None):
    ...
    with (
        ...
        dask.distributed.performance_report(filename=arguments.performance_report)
    ):
