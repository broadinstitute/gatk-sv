...
from gq_recalibrator.tarred_properties_to_parquet import PropertiesSummary
...
@attr.define(frozen=True, slots=True, weakref_slot=False)
...
class VcfTrainingTensorDataLoader(VcfTensorDataLoaderBase):
    __slots__ = (
        ...
        "max_look_ahead_batches", "_resume_generator_state", "_training_futures", "_epoch"
    )
    __state_keys__ = (
        "_resume_generator_state", "_current_parquet_file", "_parquet_files", "_current_row",
        "_training_batch", "_validation_batch", "_previous_batch_iterators", "_epoch"
    )
...
class TrainingBatchPickler(BatchPicklerBase):
    ...
    def pickle_batch_tensors(
        ...
        for is_training_batch, pickle_file, batch_number in training_batch_iterator:
            ...
            allele_counts = (
                supplemental_property_buffers[Keys.allele_count]
    ...
    def _reorganize_confident_variants(
        ...
        for sample_id, sample_confident_variants in confident_variants.items():
            sample_index = sample_index_map.get(sample_id, None)
            if sample_index is None:
                # sample ID from confident_variants is not present in VCF: can easily happen in
                # cross-validation, when withholding data, etc. Just skip this sample.
                continue
        ...
        genotype_is_good = {
            variant_id: numpy.array(sorted(sample_indices), dtype=int_dtype)
    ...
    def _fill_gt_is_good_tensor(
        ...
        for row, variant_id in enumerate(variant_ids):
            ...
            if truth_indices is not None:
                gt_is_good[row, :].put(truth_indices, True)
        # NOCALL GT is always bad
        gt_is_good[:] = numpy.logical_and(gt_is_good, allele_counts >= 0)
        if keep_homref:
    ...
    def _fill_gt_is_bad_tensor(
        ...
        for row, variant_id in enumerate(variant_ids):
            ...
            if truth_indices is not None:
                gt_is_bad[row, :].put(truth_indices, True)
        # NOCALL GT is always bad
