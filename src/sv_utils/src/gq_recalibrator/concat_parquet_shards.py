...
def concat_parquet_shards(
    ...
    output_folder = (
        output_path.with_suffix(Keys.parquet_suffix) if output_path.suffix == ".tar"
        else output_path
    )
...
def _transfer_parquet_folder(
    ...
    need_modify_parquet = _need_modify_parquet(
        input_folder=input_folder, running_parquet_stats=running_parquet_stats, weights=weights
    )
    ...
    for parquet_file in parquet_files:
        ...
        if need_modify_parquet:
            ...
            shutil.copyfile(parquet_file, new_parquet_file)
...
def _need_modify_parquet(
        input_folder: Path,
        weights: Optional[DaskSeries],
        running_parquet_stats: RunningParquetStats
) -> bool:
    f"""If the parquet files contain the "{Keys.row}" property and it's not the first shard,
    we'll need to edit the data. If we need to add computed weights, we will need to modify the
    parquet file regardless (even if weights are already present, they will be inaccurate if
    computed over individual shards). Otherwise we can just copy it.
    """
    return True if (
        (weights is not None)
        or (
            Keys.row in tarred_properties_to_parquet.get_parquet_folder_columns(
                input_path=input_folder
            ) and running_parquet_stats.num_parquet_rows > 0
        )
    ) else False
...
def _transfer_parquet_file(
    ...
    if Keys.row in df.columns:
        ...
        df.index = numpy.arange(row_start, row_end, dtype=numpy.int64)
        save_index = True
    ...
    if weights is not None:
        ...
        df = dask_utils.flatten_columns(
            tarred_properties_to_parquet.standard_sort_properties(properties_dataframe=df)
        )
