...
from gq_recalibrator import tarred_properties_to_parquet, training_utils, dask_utils
...
class VcfTensorDataLoaderBase:
    ...
    def _parquet_to_1_hot(
        ...
        df = dask_utils.flatten_columns(df)
        df.to_parquet(
    ...
    def _set_column_info(
        ...
        flat_columns, self._columns = zip(*(
            (flat_column, dask_utils.unflatten_column_name(flat_column))
    ...
    def _load_buffer(
        ...
        try:
            ...
            common.add_exception_context(
                exception,
                f"Opening {parquet_file}, "
                f"num_wanted={0 if wanted_ids is None else len(wanted_ids)}"
            )
            raise
        return buffer
