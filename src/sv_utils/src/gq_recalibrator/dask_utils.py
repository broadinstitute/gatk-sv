...
class TempPersister:
    ...
    @staticmethod
    def remove_temp_persist_folder(dask_object: DaskDataFrame | DaskSeries | DaskArray):
        """Remote temporary folders backing this dask object. Don't invoke directly."""
        persist_folder_field = TempPersister.persist_folder_field
...
def reduce_delayed(
    ...
    try:
        ...
        common.add_exception_context(exception, f"type(reduction_function): {type(reduction_function)}")
        raise
...
def add_block_value_counts(
    ...
    """
    return value_counts_1.add(value_counts_2, fill_value=0).sort_index()
