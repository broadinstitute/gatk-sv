    ...
    def get_pandas_series(self, values: Sequence[EncodedVcfField]) -> pandas.Series:
        ...
        try:
            ...
            common.add_exception_context(
                err,
                (
                    f"making series for sample '{self.sample_id}', property "
                    f"'{self.table_prop_name}', with categories '{categories}'"
                )
            )
            raise
...
class BoolPropertyCollator(VcfPropertyCollator):
    ...
    def get_dtype(self, values: Sequence[EncodedVcfField]) -> dtype:
        try:
            _has_missing = (
                any(pandas.isna(value) for value in values)
...
def shift_origin(
    ...
    begin_col = (
        next((col for col in df.columns if col[1] == Keys.begin), None)
        if isinstance(df.columns, pandas.MultiIndex)
        else Keys.begin
        if Keys.begin in df.columns
        else None
    )
    if current_origin == desired_origin or begin_col is None:
        return df
    if copy_on_change:
        df = df.copy()
...
def vcf_to_pandas(
        ...
        use_variant_id_as_index: bool = True,
        seek_pos: Optional[int] = None,
        duplicate_filehandle: bool = False,
        start_row: int = 0,
        num_take_rows: Optional[int] = None,
) -> pandas.DataFrame:
    f"""
    ...
    Args:
        ...
        use_variant_id_as_index: (Default=True) use variant_id as the index for the DataFrame if
                                 possible
        seek_pos: (Default=None) Used by vcf_to_dask to skip to a position in the VCF.
                  Don't mess with this unless you know what you're doing.
        duplicate_filehandle: (default=False) If True, tell pysam to allocate a duplicate
                              filehandle for reading in parallel. Used by vcf_to_dask.
        start_row: (Default=0) Starting row for index if not using variant ID.
        num_take_rows: (Default = None). Only take this many rows from the VCF. If None, take all
                       rows.
    Returns:
    ...
    if log_progress:
        if wanted_properties is None:
            ...
            wanted_properties = [prop.lower() for prop in wanted_properties]  # ensure lower case
            print(f"Reading {','.join(wanted_properties)} from {vcf} ...", flush=True, file=sys.stderr, end="")
    with pysam.VariantFile(
        f"{vcf}", 'r', threads=num_threads, duplicate_filehandle=duplicate_filehandle
    ) as f_in:
        if samples is None:
            samples = tuple(f_in.header.samples)
        else:
            samples: tuple[str, ...] = _get_vcf_wanted_samples(
                vcf_header=f_in.header, wanted_samples=samples,
                missing_samples_action=missing_samples_action
            )
            f_in.subset_samples(samples)

        variant_properties, genotype_properties = _get_vcf_variant_and_genotype_properties(
            vcf_header=f_in.header, wanted_properties=wanted_properties,
            missing_properties_action=missing_properties_action
        )
        ...
        if seek_pos is None:
            ...
            encoded_rows_iter = (
                tuple(_row_extractor(variant_record))
                for variant_record in itertools.islice(f_in, num_take_rows)
            )
    ...
    if (
        use_variant_id_as_index and
        variants[(None, Keys.id)].nunique() == len(variants[(None, Keys.id)])
    ):
        ...
        # use row number as the index
        variants.index = pandas.Index(
            numpy.arange(start_row, start_row + len(variants), dtype=numpy.int64), name=Keys.row
        )
...
def _get_vcf_wanted_samples(
    vcf_header: pysam.VariantHeader,
    wanted_samples: Collection[str],
    missing_samples_action: ErrorAction = Default.missing_samples_action,
) -> tuple[str, ...]:
    samples = set(wanted_samples)
    bad_samples = samples.difference(vcf_header.samples)
    if bad_samples:
        missing_samples_action.handle_error(
            f"Requested samples are not present in VCF: {bad_samples}")
    return tuple(sample for sample in vcf_header.samples if sample in samples)


def _get_vcf_variant_and_genotype_properties(
    vcf_header: pysam.VariantHeader,
    wanted_properties: Optional[Collection[str]] = None,
    missing_properties_action: ErrorAction = Default.missing_properties_action,
) -> tuple[dict[str, str], dict[str, str]]:
    """
    Get maps from property name in VCF to property name in final table/DataFrame for the
    per-variant properties and per-genotype properties.
    """
    if wanted_properties is None:
        properties = _get_all_properties(vcf_header)
    else:
        # we always want variant id, but it's easy to forget
        wanted_properties = {Keys.id}.union(wanted_properties)
        properties = _get_allowed_properties(vcf_header)
        ...
        properties = {key: value for key, value in properties.items() if
                      value in wanted_properties}

    genotype_properties = {
        vcf_prop_name: table_prop_name for vcf_prop_name, table_prop_name in properties.items()
        if vcf_prop_name in vcf_header.formats
    }
    variant_properties = {
        vcf_prop_name: table_prop_name for vcf_prop_name, table_prop_name in properties.items()
        if vcf_prop_name not in genotype_properties
    }
    return variant_properties, genotype_properties
...
def vcf_to_dask(
    ...
    Args:
        ...
        drop_missing_columns: bool (default={Default.drop_missing_columns})
            If True and an entire column is missing data, drop the column. Otherwise keep it.
            This action is performed partition-by-partition, so it may cause inconsistency if some
            partitions drop the property and some do not.
        drop_trivial_multi_index: bool (default={Default.drop_trivial_multi_index})
            If True and one of the levels of the multi-index columns is trivial (e.g. there is no format data)
            the drop the trivial level. Otherwise keep it.
        cast_whole_nums_to_int: bool (default={Default.cast_whole_nums_to_int})
        ...
        log_progress: bool (default={Default.log_progress})
            If True, display a few lines to indicate how loading the VCF is proceeding.
        dask_partition_size_mb: int (default={Default.dask_partition_size_mb}
            The desired approximate size of each partition in MB.
        num_threads: int (default={common.num_logical_cpus})
            Use this many threads for decompression while reading the VCF.
    Returns:
        variants: pandas.DataFrame
    ...
    if log_progress:
        if wanted_properties is None:
            ...
            wanted_properties = [prop.lower() for prop in wanted_properties]  # ensure lower case
            print(
                f"Reading {','.join(wanted_properties)} from {vcf} ...", flush=True,
                file=sys.stderr, end=""
            )
    with pysam.VariantFile(vcf, 'r', threads=num_threads) as vcf_in:
        if samples is None:
            samples = tuple(vcf_in.header.samples)
        else:
            if samples is None:
                samples = tuple(vcf_in.header.samples)
            else:
                samples: tuple[str, ...] = _get_vcf_wanted_samples(
                    vcf_header=vcf_in.header, wanted_samples=samples,
                    missing_samples_action=missing_samples_action
                )
                vcf_in.subset_samples(samples)
        variant_properties, genotype_properties = _get_vcf_variant_and_genotype_properties(
            vcf_header=vcf_in.header, wanted_properties=wanted_properties,
            missing_properties_action=missing_properties_action
        )

        num_samples = len(samples)
        entries_per_row = len(variant_properties) + num_samples * len(genotype_properties)
        bytes_per_entry = 8  # wild guess, probably overly conservative
        mb_per_row = bytes_per_entry * entries_per_row / (2 ** 20)
        rows_per_partition = max(1, round(dask_partition_size_mb / mb_per_row))
        if log_progress:
            print(f" (Rows per partition: {rows_per_partition}) ")
        # construct a map of the positions of partitions in the VCF file
        vcf_in.reset()
        # noinspection PyTypeChecker
        seek_map: dict[int, int] = {0: vcf_in.tell()}
        num_rows = 0
        for record_number, variant_record in enumerate(vcf_in):
            num_rows += 1
            if num_rows >= rows_per_partition:
                # noinspection PyTypeChecker
                seek_map[record_number] = vcf_in.tell()
                num_rows = 0
        dask_divisions = tuple(seek_map.keys()) + (record_number,)

    def _load_partition(
        _seek_pos: int, _start_row: int, _num_take_rows: int = rows_per_partition
    ) -> pandas.DataFrame:
        return vcf_to_pandas(
            vcf=vcf,
            genome_origin=genome_origin,
            vcf_origin=vcf_origin,
            wanted_properties=wanted_properties,
            samples=samples,
            drop_trivial_multi_index=drop_trivial_multi_index,
            ...
            missing_samples_action=missing_samples_action,
            missing_properties_action=missing_properties_action,
            category_prop_getter_ordered=category_prop_getter_ordered,
            seek_pos=_seek_pos,
            start_row=_start_row,
            num_take_rows=_num_take_rows,
            use_variant_id_as_index=False,
            num_threads=1,
            duplicate_filehandle=True,
            log_progress=False
        )

    # build list of delayed tasks for reading chunks of the VCF file
    delayed_variants = [
        dask.delayed(_load_partition)(
            _seek_pos=_seek_pos, _start_row=_start_index, _num_take_rows=rows_per_partition,
        )
        for _start_index, _seek_pos in seek_map.items()
    ]
    if reconcile_dtypes:
        delayed_variants = dask_utils.reconcile_dtypes(delayed_variants)
    # construct the dask DataFrame from the list of delayed tasks
...
def list_info_properties(variants: pandas.DataFrame) -> set[str]:
    return {col[1] for col in variants.columns if col[0] is None}


def list_format_properties(variants: pandas.DataFrame) -> set[str]:
    return set(col[1] for col in variants.columns if col[0] is not None)


def get_format_property(variants: pandas.DataFrame, property_name: str) -> pandas.DataFrame:
    """ """
    try:
        prop_df = variants[[col for col in variants.columns if col[1] == property_name]]
        # dask seemingly has problems with named MultiIndex column levels
        # prop_df.columns = prop_df.columns.droplevel(Keys.property)
        prop_df.columns = prop_df.columns.droplevel(1)
        return prop_df
...
def standard_sort_properties(
    ...
    Args:
        properties_dataframe: DataFrame to sort
        sample_ids: if known, passing sample IDs can omit recomputing them
    """
    if sample_ids is None:
        sample_ids = properties_dataframe.columns.get_level_values(level=Keys.sample_id).tolist()
    sample_order = {
        sample_id: sample_ids.index(sample_id)
        for sample_id in set(sample_ids)
        if isinstance(sample_id, str)
    }

    def _sort_columns_key(_columns: pandas.Index) -> pandas.Index:
        return pandas.Index(
            [sample_order.get(sample_id, -1) for sample_id in _columns], name=Keys.sample_id,
            dtype="string[pyarrow]"
        ) if _columns.name == Keys.sample_id else _columns

    return properties_dataframe.sort_index(
        axis=1, inplace=False, key=_sort_columns_key, level=[Keys.sample_id, Keys.property]
    ).rename_axis(index=Keys.row)
...
def assign_dataframe_column(
    ...
    if isinstance(values, pandas.Series):
        ...
        if isinstance(df.columns, pandas.MultiIndex):
            ...
            return standard_sort_properties(df)
        else:
        ...
        if not isinstance(df.columns, pandas.MultiIndex):
            # adding a multi-index property to a regular DataFrame: need to promote df to
            # multi-index
            df.columns = pandas.MultiIndex.from_product(
                [[None], df.columns], names=Default.column_levels
            )

        values.columns = pandas.MultiIndex.from_product(
            [values.columns, [name]], names=Default.column_levels
        )
        is_new_prop = name not in df.columns.get_level_values(level=Keys.property)
        if is_new_prop:
            df = pandas.concat((df, values), axis=1)
