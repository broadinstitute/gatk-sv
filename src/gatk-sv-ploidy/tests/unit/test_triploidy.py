...
def _classify_synthetic_ploidy() -> pd.DataFrame:
    ...
    return triploidy.classify_triploidy_from_site_data(
...
def test_fast_af_likelihood_matches_reference_for_masked_inputs() -> None:
    ...
    for site_pop_af in (site_pop_af_2d, site_pop_af_3d):
        for cn_state in (1, 2, 3, 4):
            ...
            observed = triploidy._summed_af_log_lik_by_concentration(
                site_alt=site_alt,
                site_total=site_total,
                site_pop_af=site_pop_af,
                site_mask=site_mask,
...
def test_classify_triploidy_from_site_data_classifies_cn1_to_cn4() -> None:
...
def test_classify_triploidy_defaults_cn4_ambiguous_half_peaks_to_diploid() -> None:
...
def test_triploidy_progress_messages_do_not_log_sample_ids(monkeypatch, caplog) -> None:
    sample_ids, bin_chr, site_alt, site_total, site_pop_af, site_mask = (
        _synthetic_ploidy_inputs()
    )
    progress_calls = []
    ...
    monkeypatch.setattr(triploidy, "tqdm", fake_tqdm)
    caplog.set_level(logging.INFO, logger=triploidy.logger.name)

    triploidy.classify_triploidy_from_site_data(
        sample_ids=sample_ids,
        bin_chr=bin_chr,
        site_alt=site_alt,
        site_total=site_total,
    ...
    log_text = "\n".join(record.getMessage() for record in caplog.records)
    assert "Triploidy progress: prepared aggregate AF matrix" in log_text
    assert "baseline_cn_states=1,2,3,4" in log_text
    assert "Triploidy progress: classification complete" in log_text
    for sample_id in sample_ids:
        assert sample_id not in log_text
