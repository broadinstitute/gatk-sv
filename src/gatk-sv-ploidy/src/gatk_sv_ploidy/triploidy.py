...
def classify_triploidy_from_site_data(
    *,
    ...
    if log_progress:
        logger.info(
            "Triploidy progress: prepared aggregate AF matrix "
    ...
    for cn_state in _BASELINE_CN_STATES:
        if log_progress:
            logger.info(
                "Triploidy progress: evaluating CN%d AF likelihood grid.",
                cn_state,
            )
        ...
        if log_progress:
            logger.info(
                "Triploidy progress: CN%d AF likelihood grid complete "
    ...
    if log_progress:
        logger.info("Triploidy progress: evaluating direct AF peak mixtures.")
    ...
    if log_progress:
        logger.info("Triploidy progress: classifying samples.")
    ...
    for sample_idx in sample_indices:
        ...
        if n_blocks < min_informative_bins:
            ...
            if supported_states:
                ...
                call = _BASELINE_CN_LABELS[baseline_cn]
        ...
        row: dict[str, object] = {
            ...
            "baseline_cn_map_call": _BASELINE_CN_LABELS[int(map_cn)],
    ...
    if log_progress:
        logger.info(
            "Triploidy progress: classification complete %s",
        ...
        logger.info(
            "Triploidy progress: classification call summary %s",
            _format_category_counts(
...
def _log_privacy_safe_threshold_counts(
    ...
    for cn_state in _NON_DIPLOID_CN_STATES:
        ...
        logger.info(
            "PRIVACY_SAFE_TRIPLOIDY posterior_threshold_counts state=cn%d "
    ...
    logger.info(
        "PRIVACY_SAFE_TRIPLOIDY tetraploid_ambiguity_counts "
...
def _log_privacy_safe_decision_margins(results_df: pd.DataFrame) -> None:
    for cn_state in _NON_DIPLOID_CN_STATES:
        ...
        logger.info(
            "PRIVACY_SAFE_TRIPLOIDY decision_margin_summary state=cn%d "
...
def _log_privacy_safe_concentration_boundaries(results_df: pd.DataFrame) -> None:
    ...
    for cn_state in _BASELINE_CN_STATES:
        ...
        for label, mask in (
            ...
            logger.info(
                "PRIVACY_SAFE_TRIPLOIDY concentration_boundary_counts "
        ...
        logger.info(
            "PRIVACY_SAFE_TRIPLOIDY concentration_mean_quantiles state=cn%d %s",
            cn_state,
...
def _select_diploid_diagnostic_samples(
    ...
    diploid_df = metrics_df[
        ...
        (metrics_df["baseline_cn_call"] == "DIPLOID")
    ].copy()
    if diploid_df.empty:
        return []
    diploid_df["_ambiguous_sort"] = diploid_df[
        ["posterior_cn_1", "posterior_cn_3", "posterior_cn_4"]
    ].max(axis=1)
    diploid_df["_site_sort"] = diploid_df["diagnostic_informative_sites"].fillna(0)
    diploid_df = diploid_df.sort_values(
        ["_ambiguous_sort", "_site_sort", "sample"],
...
def write_triploidy_diagnostics(
    *,
    ...
    logger.info("Triploidy progress: computing diagnostic AF metrics.")
    ...
    logger.info(
        "Triploidy progress: sampling diagnostic AF sites selected_samples=%d.",
    ...
    logger.info(
        "Triploidy progress: sampling diploid comparison AF sites "
        "selected_samples=%d.",
        int(len(diploid_samples)),
    )
    ...
    logger.info("Triploidy progress: rendering diagnostic plots.")
...
def main() -> None:
    ...
    logger.info("Triploidy progress: loading depth table and site data.")
    ...
    logger.info(
        "Triploidy progress: loaded aggregate inputs samples=%d bins=%d "
    ...
    logger.info("Triploidy progress: starting autosomal baseline CN classification.")
    ...
    if args.privacy_safe_diagnostics:
        logger.info("Triploidy progress: computing privacy-safe AF diagnostics.")
    ...
    logger.info("Triploidy progress: logging aggregate privacy-safe summary.")
    ...
    logger.info("Triploidy progress: writing result tables.")
