...
function MergeSample() {
  ...
  if [[ -n "${_std_cnmops}${_large_cnmops}" ]]; then
      zcat ${_std_cnmops} ${_large_cnmops} | awk -F "\t" -v OFS="\t" -v sample="${_sample_id}" '{if ($5==sample) print}' > cnmops.cnv
  fi

  cat ${_gcnv} ${_dragen} cnmops.cnv | sort -k1,1V -k2,2n > "${_sample_id}.bed"
...
run_cnmops="false"
if [[ -n "${std_cnmops_del}" || -n "${std_cnmops_dup}" || -n "${large_cnmops_del}" || -n "${large_cnmops_dup}" ]]; then
  run_cnmops="true"
fi
...
if [[ "${run_dragen}" == "true" ]]; then
  python /opt/sv-pipeline/scripts/convert_dragen_vcf_to_bed.py "${dragen_cnv_vcf}" "${batch}"
  dragen_del_bed="$(realpath ${batch}.del.bed)"
  dragen_dup_bed="$(realpath ${batch}.dup.bed)"
fi
...
for sample in "${samples[@]}"; do
  ...
  MergeSample "${sample}" "${std_cnmops_del}" "${large_cnmops_del}" "${gcnv_del}" "${dragen_del_bed}" "${defragment_max_dist}"
...
for sample in "${samples[@]}"; do
  ...
  MergeSample "${sample}" "${std_cnmops_dup}" "${large_cnmops_dup}" "${gcnv_dup}" "${dragen_dup_bed}" "${defragment_max_dist}"
...
working_dir="$(realpath ${working_dir})"
cd "${working_dir}"
# note that "merged_dup_beds" in the following is passing the array, not a string.
