...
if [[ "${run_scramble}" == true && "${run_manta_or_dragen}" == true ]]; then
...

  {
    ...
    jq -n \
      ...
      --arg alignment_score_cutoff "${scramble_alignment_score_cutoff}" \
      --arg mei_bed "${mei_bed}" \
      '{
          ...
          alignment_score_cutoff: ($alignment_score_cutoff | tonumber),
          mei_bed: $mei_bed
      }' > "${scramble_p1_inputs_json_filename}"
...
#    echo "Running Scramble part 2."
    ...
    scramble_p2_inputs_json_filename=$(mktemp --suffix=.json "${output_dir}/scramble_p2_inputs_XXXXXX")
    jq -n \
      --arg sample_name "${sample_id}" \
      --arg bam_or_cram_file "$(jq -r ".out" "${realign_soft_clipped_reads_json_filename}")" \
      ...
      '{
          ...
          alignment_score_cutoff: ($alignment_score_cutoff | tonumber),
          mei_bed: $mei_bed
      }' > "${scramble_p2_inputs_json_filename}"
