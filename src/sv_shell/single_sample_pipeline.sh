...
cd "${working_dir}"
SamplesList_wd=$(mktemp -d /wd_SamplesList_XXXXXXXX)
SamplesList_wd="$(realpath "${SamplesList_wd}")"
cd "${SamplesList_wd}"

combined_ped_file=$(jq -r ".combined_ped_file" "$gather_batch_evidence_outputs_json_filename")
awk -v sex=1 '($5==sex) {print $2}' "${combined_ped_file}" > ped_males.list
awk -v sex=2 '($5==sex) {print $2}' "${combined_ped_file}" > ped_females.list
printf "%s\n" "${sample_id}" "${ref_samples[@]}" > samples.list
