...
class Criterion:
    ...
    def test(self, value):
        if not self.is_testable():
            raise ValueError("Criterion %s could not be tested because it does not have assigned bounds" % self.name)
...
class QCEvaluator:
    ...
    def _load_metrics(self, filename):
        ...
        self.data = pd.read_csv(filename, sep=const.TSV_DELIMITER, names=cols, header=None)

    def _load_criteria(self, filename):
        cols = [const.CRITERIA_METRIC_COLUMN, const.CRITERIA_MIN_COLUMN, const.CRITERIA_MAX_COLUMN]
        raw_data = pd.read_csv(filename, sep=const.TSV_DELIMITER, names=cols, header=0)
        self.criteria = {}
    ...
    def get_tests(self):
        ...
        missed_tests = [Test(self.criteria[x]) for x in self.criteria if x not in tests]
    ...
    def write_tests(self, filename, tests):
        with open(filename, 'w') as f:
            ...
            f.writelines(["\t".join([x.criterion.name, x.criterion.min_string(), x.criterion.max_string(), x.get_value_string(), x.result]) + "\n" for x in tests])
