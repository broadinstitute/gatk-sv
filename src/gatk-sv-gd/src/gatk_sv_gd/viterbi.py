...
# =============================================================================
# Non-NAHR Viterbi-based GD CNV calling
# =============================================================================


def viterbi_call_non_nahr_cnv(
    locus: GDLocus,
    sample_probs: np.ndarray,
    transition_matrix: np.ndarray,
    interval_bin_arrays: Dict[str, np.ndarray],
    ...
    Non-NAHR loci have variable breakpoints (mediated by NHEJ, FoSTeS/MMBIR)
    so the calling logic differs from NAHR:

    - **No breakpoint transition matrix** — breakpoints are not recurrent, so
      there is no special transition at known SD-block boundaries.
    - **Fraction-based calling** — instead of requiring ALL bins in the covered
      region to show variant CN, we require at least *min_variant_fraction*
      of the body bins to be variant.  This accommodates events that may span
      only a portion of the defined GD region.
    - **Flank checking** is the same as for NAHR.

    Args:
        locus: GDLocus object (``is_nahr == False``).
        sample_probs: (n_bins, n_states) posterior probabilities for one
            sample across all bins at this locus.
        transition_matrix: (n_states, n_states) transition matrix.
        interval_bin_arrays: Dict mapping interval name to array of bin
            array_idx values.
        ploidy: Expected reference copy number.
        confidence_threshold: Minimum mean per-bin log-posterior along the
            Viterbi path to make a carrier call.
    ...
    Returns:
        List of call dicts (one per GD entry).
    """
    all_bins = sorted(set(int(b) for bins in interval_bin_arrays.values() for b in bins))
    if not all_bins:
        if verbose:
            _util.vlog(f"  [VIT-NN] {sample_id}  {locus.cluster}: no bins -> skip")
        return []

    n_states = sample_probs.shape[1]
    priors = np.maximum(sample_probs[all_bins], 1e-30)
    state_log_priors = np.log(priors)
    tm = transition_matrix[:n_states, :n_states]
    ...
    path, confidence = run_viterbi(state_log_priors, tm, initial_log_probs)

    if verbose:
        segments = _extract_segments(path)
        seg_str = " -> ".join(f"CN{s}x{e - st}" for st, e, s in segments)
        conf_pass = "PASS" if confidence > confidence_threshold else "FAIL"
        _util.vlog(f"  [VIT-NN] {sample_id}  {locus.cluster}  "
                   f"{len(all_bins)} bins  ploidy={ploidy}  "
                   f"confidence={confidence:+.4f} [{conf_pass}]")
        _util.vlog(f"    Segments: {seg_str}")

    # Map path back to intervals
    local_idx = {b: i for i, b in enumerate(all_bins)}
    interval_bin_states: Dict[str, np.ndarray] = {}
    for name, bin_idx in interval_bin_arrays.items():
        local = [local_idx[int(b)] for b in bin_idx if int(b) in local_idx]
        interval_bin_states[name] = path[local] if local else np.array([], dtype=int)

    flank_names = {name for _, _, name in locus.get_flanking_regions()}
    body_names = set(interval_bin_arrays.keys()) - flank_names

    calls = []
    for entry in locus.gd_entries:
        gd_id = entry["GD_ID"]
        svtype = entry["svtype"]
        bp1, bp2 = entry["BP1"], entry["BP2"]

        covered_tuples = locus.get_intervals_between(bp1, bp2)
        covered = set(name for _, _, name in covered_tuples)
        if not covered:
            print(f"  WARNING [VIT-NN]: {gd_id} ({svtype}) BP1={bp1}, BP2={bp2} "
        ...
        if verbose:
            ...
            status = "PASS" if covered_ok else "FAIL"
        ...
        for iv in uncovered_body | flank_names:
            ...
            if svtype == "DEL":
                ...
                continue
            if ok_frac < flank_coverage_threshold:
                flanks_ok = False
                if verbose:
                    _util.vlog(f"      Flank {iv}: {ok_frac:.0%} at ref CN "
