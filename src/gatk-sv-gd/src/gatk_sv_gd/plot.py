...
def _merge_segments(segments: List[dict]) -> List[dict]:
    """Merge consecutive segments with the same category into single spans.

    Input segments must be sorted by ``start``.  Adjacent segments whose
    categories match are fused: the merged segment inherits the earliest
    ``start``, latest ``end``, and a weighted-average ``mean_cn`` (weighted
    by genomic span).  This guarantees every bin is covered by exactly one
    output segment with no gaps or overlaps between same-category runs.
    """
    if not segments:
        return []
    ...
    for seg in segments[1:]:
        ...
        if seg["category"] == prev["category"]:
            # Weighted average CN by genomic span
            prev_span = max(prev["end"] - prev["start"], 1)
            seg_span = max(seg["end"] - seg["start"], 1)
            total_span = prev_span + seg_span
            prev["mean_cn"] = (
                prev["mean_cn"] * prev_span + seg["mean_cn"] * seg_span
            ) / total_span
            prev["end"] = seg["end"]
        else:
...
class ViterbiOverlayData:
    ...
    def __init__(self, paths_df: pd.DataFrame):
        # Support files generated by the old code that stored per-bin
        # cn_state instead of per-segment mean_cn/category.  Upgrade on
        # the fly by deriving category assuming ploidy=2.
        if "mean_cn" not in paths_df.columns:
            if "cn_state" not in paths_df.columns:
                raise ValueError(
                    "viterbi_paths.tsv.gz has neither 'mean_cn' nor 'cn_state' "
                    "columns.  Please re-run gd_cnv_call.py to regenerate it."
                )
            print(
                "  WARNING: viterbi_paths.tsv.gz uses the old per-bin format "
                "(cn_state column).  Re-run gd_cnv_call.py to get the new "
                "segment format.  Falling back to per-bin compatibility mode "
                "with ploidy=2 assumed."
            )
            paths_df = paths_df.copy()
            paths_df["mean_cn"] = paths_df["cn_state"].astype(float)
            paths_df["category"] = paths_df["cn_state"].apply(
                lambda cn: "DEL" if cn < 2 else ("DUP" if cn > 2 else "REF")
            )

        # Index by (sample, cluster) for O(1) lookup.
        ...
        # no gaps or overlaps.
        self._paths: Dict[Tuple[str, str], List[dict]] = {}
        for (sample, cluster), grp in paths_df.groupby(["sample", "cluster"]):
            rows = grp.sort_values("start")
            merged = _merge_segments([
                {
                    "start": int(row["start"]),
                    "end": int(row["end"]),
                    "mean_cn": float(row["mean_cn"]),
                    "category": str(row["category"]),
                }
                for _, row in rows.iterrows()
            ])
            self._paths[(str(sample), str(cluster))] = merged
    ...
    def get_path(
        ...
        """Look up the pre-computed Viterbi category segments for one sample/cluster.

        Returns a list of segment dicts with keys ``start``, ``end``,
        ``mean_cn``, and ``category`` (DEL/REF/DUP), or *None* if no
        path was saved for this combination.
        """
        return self._paths.get((str(sample_id), str(cluster)))
                    ...
                    if vit_segs:
                        ...
                        for i, seg in enumerate(vit_segs):
                            ...
                            vit_x.extend([xform(seg["start"]),
                                          xform(seg["end"])])
                            vit_y.extend([seg["mean_cn"], seg["mean_cn"]])
                        ax.plot(vit_x, vit_y,
