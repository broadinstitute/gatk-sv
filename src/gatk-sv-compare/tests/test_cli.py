...
def test_cli_validate_fix_suggests_drop_bnd(make_vcf, capsys) -> None:
    vcf_path = make_vcf(
    ...
    assert "--drop-bnd" in out


def test_cli_validate_fix_drops_all_bnd_when_requested(make_vcf, tmp_path, capsys) -> None:
    vcf_path = make_vcf(
        file_name="drop_bnd_cli.vcf",
        records=[
            "chr1\t100\tvar1\tN\t<BND>\t.\tPASS\tSVTYPE=BND\tGT:GQ:ECN\t0/1:60:2\t0/0:50:2",
    ...
    out_path = tmp_path / "drop_bnd_cli.fixed.vcf.gz"

    exit_code = main(["validate", "--vcf", str(vcf_path), "--fix", "--drop-bnd", "--out", str(out_path)])
...
def test_cli_validate_fix_repairs_legacy_bnd_end_to_end2(make_vcf, tmp_path, capsys) -> None:
    vcf_path = make_vcf(
        file_name="legacy_bnd_end_cli.vcf",
        records=[
            "chr1\t100\tvar1\tN\t<BND>\t.\tPASS\tSVTYPE=BND;CHR2=chr2;END=90\tGT:GQ:ECN\t0/1:60:2\t0/0:50:2",
        ],
    )
    out_path = tmp_path / "legacy_bnd_end_cli.fixed.vcf.gz"
...
def test_cli_validate_fix_suggests_drop_ctx(make_vcf, capsys) -> None:
    vcf_path = make_vcf(
    ...
    assert "--drop-ctx" in out


def test_cli_validate_fix_drops_all_ctx_when_requested(make_vcf, tmp_path, capsys) -> None:
    vcf_path = make_vcf(
        file_name="drop_ctx_cli.vcf",
        records=[
            "chr1\t100\tvar1\tN\t<CTX>\t.\tPASS\tSVTYPE=CTX\tGT:GQ:ECN\t0/1:60:2\t0/0:50:2",
    ...
    out_path = tmp_path / "drop_ctx_cli.fixed.vcf.gz"

    exit_code = main(["validate", "--vcf", str(vcf_path), "--fix", "--drop-ctx", "--out", str(out_path)])
...
def test_cli_validate_fix_repairs_legacy_ctx_end_to_end2(make_vcf, tmp_path, capsys) -> None:
    vcf_path = make_vcf(
        file_name="legacy_ctx_end_cli.vcf",
        records=[
            "chr1\t100\tvar1\tN\t<CTX>\t.\tPASS\tSVTYPE=CTX;CHR2=chr2;END=90\tGT:GQ:ECN\t0/1:60:2\t0/0:50:2",
        ],
    )
    out_path = tmp_path / "legacy_ctx_end_cli.fixed.vcf.gz"
