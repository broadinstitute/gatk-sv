    ...
    for i in sv_types:
        for j in stat_hash.keys():
            if i in stat_hash[j].keys():
                print('\t'.join([str(k) for k in [j, i, stat_hash[j][i], sample_name]]), file=fo)
            else:
                print('\t'.join([str(k) for k in [j, i, 0, sample_name]]), file=fo)
    fo.close()

def main():
    parser = argparse.ArgumentParser("calcu_num_SVs.by_type_chromo.py")
    parser.add_argument("vcf", type=str, help="name of input file in vcf.gz format")
