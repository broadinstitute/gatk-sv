...
out_size=100
if args.size:   out_size=int(args.size)
print(out_size)
info=[]
fin=open(file_name)
for line in fin:
        pin=line.strip().split()
        if not pin[0][0]=='#':  info.append(pin)
fin.close()
out=[i for i in range(len(info))]
random.shuffle(out)
out2=[]
for i in out:
        if out2==[]:    out2.append([i])
        elif len(out2[-1])<out_size: out2[-1].append(i)
        else:   out2.append([i])
...
for i in out2:
        ...
        fo=open(fileout,'w')
        for j in tmp:   
            if int(j[2])-int(j[1])<1000000 and int(j[2])-int(j[1])>0:        print('\t'.join(j), file=fo)
        fo.close()
