...
def flag_record(record):
	...
	out='True'
	if record[1] in ['NA', '0.0'] and record[2] in ['NA', '0.0'] and record[3] in ['NA', '0.0']:
		out='False'
	return out

def merge_info(fe_info,ma_info):
	out=[fe_info[0]]
	out_key=unify_list([i[0] for i in fe_info[1:]]+[i[0] for i in ma_info[1:]])
	out_hash={}
	for i in fe_info:
		out_hash[i[0]]=[]
	...
	for i in out_hash.keys():
		if len(out_hash[i])==1: continue
		elif flag_record(out_hash[i][0])=='False': out_hash[i]=[out_hash[i][1]]
		else:	out_hash[i]=[out_hash[i][0]]
	for i in out_key:
		out+=out_hash[i]
	return out
...
if args.chrom == 'Y': 
	file_in='petest_allosomes/'+'.'.join([args.batch, args.source, args.chrom, 'males.stats'])
	file_to='petest/'+'.'.join([args.batch, args.source, args.chrom, 'stats'])
	os.system(r'''cp %s %s'''%(file_in, file_to))
elif args.chrom == 'X':
	female_in='petest_allosomes/'+'.'.join([args.batch, args.source, args.chrom, 'females.stats'])
	male_in='petest_allosomes/'+'.'.join([args.batch, args.source, args.chrom, 'males.stats'])
	file_to='petest/'+'.'.join([args.batch, args.source, args.chrom, 'stats'])
