...
if args.chrom == 'Y':
        file_in='srtest_allosomes/'+'.'.join([args.batch, args.source, args.chrom, 'males.stats'])
        file_to='srtest/'+'.'.join([args.batch, args.source, args.chrom, 'stats'])
        os.system(r'''cp %s %s'''%(file_in, file_to))
elif args.chrom == 'X':
        female_in='srtest_allosomes/'+'.'.join([args.batch, args.source, args.chrom, 'females.stats'])
        male_in='srtest_allosomes/'+'.'.join([args.batch, args.source, args.chrom, 'males.stats'])
        file_to='srtest/'+'.'.join([args.batch, args.source, args.chrom, 'stats'])
