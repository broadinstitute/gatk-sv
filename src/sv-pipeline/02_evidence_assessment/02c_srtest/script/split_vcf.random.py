...
def filein_readin(filein,file_size):
        ...
        for line in fin:
                ...
                elif pin[0][0]=='#': header2.append(pin)
                else:
                    info.append(pin)
        ...
        info_new=[]
        for i in info_index:
            if info_new==[]:    info_new.append([info[i]])
            elif len(info_new[-1])<file_size:    info_new[-1].append(info[i])
            else:   info_new.append([info[i]])
        return [header, header2, info_new]

def fileout_write(header, header2, info_sub, fileout):
        fo=open(fileout,'w')
        for i in header:        print(' '.join(i), file=fo)
        for i in header2:       print('\t'.join(i), file=fo)
        for i in info_sub:      print('\t'.join(i), file=fo)
        fo.close()
...
def main():
        ...
        parser.add_argument("input", type=str, help="namd of input vcf.gz to be splited")
