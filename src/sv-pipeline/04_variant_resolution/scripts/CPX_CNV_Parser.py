...
def extract_non_ref_samples(pin, sample_name):
	...
	sample_out=[i for i in sample_name[1:] if not sample_GT[sample_name.index(i)-1]=='0/0' ]
	return ','.join(sample_out)

def write_output(final_out, fileout):
	fo=open(fileout, 'w')
	for i in final_out:
		i_new=[i[0].split('_')[1].split(':')[0]]+i[0].split('_')[1].split(':')[1].split('-')+i[1:]
	...
	for line in fin:
		...
		if not pin[0][0]=='#':
			...
			if info_cha_extract(i, 'CPX_TYPE')=='DUP5/INS3':
				#take out duplicated region in DUP5/INS3
				final_out.append([info_cha_extract(i, 'SOURCE'),i[2], extract_non_ref_samples(i, sample_name), 'DUP'])	
			...
			if info_cha_extract(i, 'CPX_TYPE') in ['delINV','delINVdel','INVdel','dupINV','INVdup','dupINVdup','delINVdup']:
				...
				for j in intervals:
					final_out.append([j,i[2], extract_non_ref_samples(i, sample_name), j.split('_')[0]])
...
def main():
	parser = argparse.ArgumentParser(description='Script to extract CNVs involved in complex SVs.')
