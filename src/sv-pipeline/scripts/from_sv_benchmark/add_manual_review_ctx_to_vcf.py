...
def create_ctx_record(new_idx: int,
    ...
    for sample in new_record.samples.keys():
        if sample in samples:
            ...
            populate_ref_gt(new_record, row, sample)
    new_record.info['END2'] = int(row["info_END2"])
...
def populate_ref_gt(new_record, row, sample):
...
def revise_record_gt(new_record):
    for sample in new_record.samples.keys():
        if new_record.samples[sample]['GT'] == (None,):
            new_record.samples[sample]['GT']=(0, 0)
            new_record.samples[sample]['GQ']=0
    return new_record
