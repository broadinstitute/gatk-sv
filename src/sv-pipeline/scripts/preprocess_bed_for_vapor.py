...
def reformat(bed_file, contig, sample_to_extract):
    ...
    found_contig = False
    found_sample = False
    with open_fn(bed_file, open_mode) as inp, open(f"{contig}.vapor.bed", 'w') as out:
        for line in inp:
...
def main():
    ...
    parser.add_argument("bed_file", help="Input SV BED file")
    ...
    reformat(args.bed_file, args.contig, args.sample)
