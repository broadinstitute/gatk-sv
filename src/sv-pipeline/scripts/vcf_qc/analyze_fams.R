...
getFamDat <- function(dat, perSampDir, proband, father=NA, mother=NA, biallelic=T,
                      nocall.placeholder=9999, max.GQ=99){
  #Clean parent IDs
  if(is.na(father)){
    father <- NULL
  }
  ...
  VID.lists <- lapply(c(proband, father, mother), readDatPerSample, perSampDir=perSampDir)
  ...
  dat.fam <- subsetDat(dat=dat, vlist=trans, biallelic=biallelic,
                       max.GQ.pro=max.GQ, max.GQ.par=max.GQ)
  dat.fam[, (ncol(dat.fam)-9):ncol(dat.fam)] <-
    apply(dat.fam[, (ncol(dat.fam)-9):ncol(dat.fam)], 2, as.numeric)
  return(dat.fam)
}
  ...
  DNRs <- as.data.frame(t(sapply(1:nrow(size.df), function(s){
    ...
    sapply(1:nrow(freq.df), function(f){
      ...
      DNR <- deNovoRateByClass(trio.dat.list,
                               VIDs=dat$VID[which(dat$VID %in% VIDs
                                                  & dat$length>size.df[s, 1]
                                                  & dat$length<=size.df[s, 2]
                                                  & dat[, freq.idx]>freq.df[f, 1]
                                                  & dat[, freq.idx]<=freq.df[f, 2])])
...
plotInhStats <- function(inh.stats, count="variants", title=NULL, cex.lab=1){
  ...
  if(count=="variants"){
    total.trans <- inh.stats$fa.site.trans+inh.stats$mo.site.trans
    total.all <- inh.stats$fa.site.all+inh.stats$mo.site.all
    plot.df <- data.frame(inh.stats$pro.site.inhrate,
                          ...
                          "total.site.transrate"=total.trans/total.all,
                          inh.stats$fa.site.transrate,
                          inh.stats$mo.site.transrate)
  }else{
    total.trans <- inh.stats$fa.allele.trans+inh.stats$mo.allele.trans
    total.all <- inh.stats$fa.allele.all+inh.stats$mo.allele.all
    plot.df <- data.frame(inh.stats$pro.allele.inhrate,
                          ...
                          "total.allele.transrate"=total.trans/total.all,
...
prepSizePlot <- function(xlims=c(50, 1000000), cex.lab=1){
  ...
  logscale.minor.labs <- as.character(sapply(c("bp", "kb", "Mb"), function(suf){
    paste(c(1, 5, 10, 50, 100, 500), suf, sep="")
  }))
...
plotDNRvsSize <- function(DNRs, bins, k=4, title=NULL, legend=T,
  ...
  mtext(3, line=0.2, cex=0.8*cex.lab,
        text=paste("n=", prettyNum(nfams, big.mark=","),
                   " ", fam.type, "s", sep=""))
...
plotHeatmap <- function(mat, nfams, fam.type,
  ...
  axis(2, at=-(1:nrow(mat))+0.5, tick=F, line=-0.8, las=2,
       labels=y.labels, cex.axis=0.7)
  # mtext(2, line=2.75, text=y.title, cex=cex.lab)
  mtext(1, line=0, cex=0.7*cex.lab,
        text=paste("Median of N=", prettyNum(nfams, big.mark=","),
                   " ", fam.type, "s", sep=""))
...
wrapperInheritancePlots <- function(fam.dat.list, fam.type, count="variants"){
  ...
  plotInhStats(inh.stats=computeInheritanceMulti(trio.dat.list=fam.dat.list,
                                                 VIDs=NULL),
               title=paste(title.prefix, "Inheritance [All SV] ",
                           title.suffix, sep=""),
               count=count)
  dev.off()
  ...
  sapply(svtypes$svtype, function(svtype){
    ...
    plotInhStats(inh.stats=computeInheritanceMulti(trio.dat.list=fam.dat.list,
                 ...
                 title=paste(title.prefix, "Inheritance [", svtype, "] ",
                             title.suffix, sep=""),
                 count=count)
    dev.off()
  })
  ...
  pdf(paste(OUTDIR, "/supporting_plots/sv_inheritance_plots/sv_inheritance.",
      ...
      height=3.75, width=4.5)
  plotInhStats(inh.stats=computeInheritanceMulti(trio.dat.list=fam.dat.list,
                                                 VIDs=dat$VID[which(dat$length>medlarge.max.size
                                                                    & dat$length<=large.max.size)]),
  ...
  plotInhStats(inh.stats=computeInheritanceMulti(trio.dat.list=fam.dat.list,
               ...
               title=paste(title.prefix, "Inheritance [", freq.lab, "<1%] ",
                           title.suffix, sep=""),
               count=count)
  dev.off()
  #Uncommon
  ...
  plotInhStats(inh.stats=computeInheritanceMulti(trio.dat.list=fam.dat.list,
                                                 VIDs=dat$VID[which(dat[, freq.idx]>rare.max.freq
                                                                    & dat[, freq.idx]<=uncommon.max.freq)]),
               title=paste(title.prefix, "Inheritance [", freq.lab, " 1-10%] ",
                           title.suffix, sep=""),
               count=count)
  dev.off()
  #Common
  pdf(paste(OUTDIR, "/supporting_plots/sv_inheritance_plots/sv_inheritance.",
      ...
      height=3.75, width=4.5)
  plotInhStats(inh.stats=computeInheritanceMulti(trio.dat.list=fam.dat.list,
                                                 VIDs=dat$VID[which(dat[, freq.idx]>uncommon.max.freq
                                                                    & dat[, freq.idx]<=common.max.freq)]),
               title=paste(title.prefix, "Inheritance [", freq.lab,
                           " 10-50%] ", title.suffix, sep=""),
               count=count)
  dev.off()
  #Major
  pdf(paste(OUTDIR, "/supporting_plots/sv_inheritance_plots/sv_inheritance.",
  ...
  plotInhStats(inh.stats=computeInheritanceMulti(trio.dat.list=fam.dat.list,
               ...
               title=paste(title.prefix, "Inheritance [", freq.lab, ">50%] ",
                           title.suffix, sep=""),
               count=count)
  dev.off()
}
...
wrapperDeNovoRateLines <- function(fam.dat.list, fam.type, count="variants",
  ...
  # Never generate GQ plots, regardless of the gq parameter
}
...
masterInhWrapper <- function(fam.dat.list, fam.type, gq=T, max.GQ=99){
  ...
  #Prepare plot area - always use the layout without GQ plots regardless of gq parameter
  width <- 10
...
if(nrow(trios)>0){
  ...
  trio.dat <- apply(trios[, 2:4], 1, function(IDs){
    ...
    return(getFamDat(dat=dat, perSampDir=perSampDir,
                     proband=IDs[1], father=IDs[2], mother=IDs[3],
                     biallelic=!multiallelics, max.GQ=maxgq))
  })
  names(trio.dat) <- trios[,1]
 
  # GQ values are still used for filtering but we never generate GQ plots
 
  #Master wrapper
  masterInhWrapper(fam.dat.list=trio.dat, fam.type="trio", gq=FALSE, max.GQ=maxgq)
