...
class GDRMatch:
    def __init__(self, name, chrom, pos, end, sample, svtype, overlapping_region_intervals,
        ...
        self.region = region
        self.genotype_calls = genotype_calls
...
class GenotypeCall:
    def __init__(self, call, threshold, hom_ref_cn, pos, stop):
        self.call = call
...
def get_n_alt_alleles(genotype_call):
    ...
    with pysam.VariantFile(vcf_path) as vcf:
        ...
        for record in vcf:
            ...
            get_revisions_over_regions(
                ...
                samples=record.samples, gdr_trees=gdr_trees, genotypes=genotypes, cutoff=cutoff,
    ...
    for svtype in region_intervals_dict:
        for region_name in region_intervals_dict[svtype]:
            ...
            get_revisions_over_regions(
                ...
                gdr_trees=gdr_trees, genotypes=genotypes, cutoff=cutoff,
                region_intervals_dict=region_intervals_dict,
                min_supported_valid_overlapping_intervals_frac=min_supported_valid_overlapping_intervals_frac,
                min_region_overlap=min_region_overlap
            )
...
def get_revisions_over_regions(regions, false_negative_matches, false_positive_matches, true_positives,
                               ...
                               genotypes, cutoff, region_intervals_dict, min_supported_valid_overlapping_intervals_frac,
                               min_region_overlap):
    ...
    for region in regions:
        ...
        for sample in samples:
            ...
            for interval in region_padded_sufficiently_overlapping_intersect:
                ...
                if sample in genotypes and interval_name in genotypes[sample]:
                    ...
                    genotype_calls.append(genotypes[sample][interval_name])
...
def read_geno_file(list_path, del_ids, dup_ids, ploidy_table_dict, par_trees,
                     min_par_overlap):
    ...
    for path in file_paths:
        with open(path) as f:
            ...
            for line in f:
                ...
                genos = [float(m) for m in record[4:]]
                for i, g in enumerate(genos):
                    ...
                    if expected_cn == 0:
                        continue
                    if is_del and g >= expected_cn:
                        continue
                    elif (not is_del) and g <= expected_cn:
                        continue
...
def get_revised_intervals(sample_overlappers, regions, pos, stop, dangling_fraction):
    ...
    for region in regions:
        ...
        for interval in invalidated_intervals:
...
def manifest_record(chrom, pos, stop, new_id, old_id, old_pos, old_stop, svtype, region, sample, batch, code):
    return f"{chrom}\t{pos}\t{stop}\t{new_id}\t{old_id}\t{old_pos}\t{old_stop}\t" \
           f"{svtype}\t{region}\t{sample}\t{batch}\t{code}\n"
...
def write_revised_variant_record(f_revise_after, f_manifest, interval, index, base_record, vid, samples,
    ...
    for sample in samples:
        for region in regions:
            f_manifest.write(manifest_record(chrom=new_record.chrom, pos=new_record.pos, stop=new_record.stop,
                                             new_id=new_record.id, old_id=vid, old_pos=old_pos, old_stop=old_stop,
...
def rescue_format_fields(gt, sample, chrom, svtype, genotype_calls, rescue_genotype, reset_pesr, ploidy_table_dict):
    ...
    genotype_alt_counts = sorted(genotype_calls, key=get_n_alt_alleles)
    ...
    for vcf_record in f_before:
        ...
        for sample, sample_overlappers in groupby(overlappers, key=attrgetter("sample")):
            ...
            for interval in new_intervals:
                intervals_dict[(interval[0], interval[1])].append(sample)
        ...
        for sample, sample_dict in svtype_dict.items():
            for _, record_list in sample_dict.items():
                ...
                new_intervals = get_revised_intervals(sample_overlappers=[record], regions=[region],
                                                      pos=padded_region_interval.begin,
                                                      stop=padded_region_interval.end,
                                                      dangling_fraction=0)
                tree = IntervalTree()
                for interval in new_intervals:
                    tree.add(interval)
                ...
                if len(reciprocal_overlappers) > 0:
                    ...
                    if (new_interval.end - new_interval.begin) > (
                            max_new_variant_size * (region_interval.end - region_interval.begin)):
                        continue
                    new_records = list()
                    ...
                    if svtype in true_positives and sample in true_positives[svtype] \
                            and region in true_positives[svtype][sample]:
                        for tp_interval in true_positives[svtype][sample][region]:
                            if reciprocal_overlap(new_interval, tp_interval) >= min_true_positive_reciprocal_overlap \
                                    or overlap_size(region_interval, tp_interval) / (
                                    region_interval[1] - region_interval[0]) >= min_new_variant_region_overlap:
                                is_true_positive = True
                                break
                    if (not is_rescued) and (not is_true_positive):
                        # Subset genotypes to the interval
                        genotype_calls = [g for g in record.genotype_calls
        ...
        with pysam.VariantFile(revised_records_before_update_path) as f_before, \
            ...
            new_records_dict = \
                adjudicate_raw_region_false_negatives(
                    ...
                    max_new_variant_size=max_new_variant_size
                )
...
def _parse_arguments(argv: List[Text]) -> argparse.Namespace:
    ...
    parser.add_argument('--geno-file-list', type=str, required=True,
                        help='List of paths to RDTest ".geno" files')
    ...
    parser.add_argument('--min-region-overlap', type=float, default=0.4,
    ...
    parser.add_argument('--min-false-negative-rescue-overlap', type=float, default=0.8,
                        help='Min overlap fraction of a variant to rescue false negatives in an existing call. '
                             'If not met, a new variant will be created. Also controls overlap required for a'
                             'false negative genotype to be denied revision if there is a similar true positive.')
    ...
    parser.add_argument('--min-new-variant-region-overlap', type=float, default=0.5,
                        help='Min overlap over a region for a new variant to be created during raw region rescue. '
                             'Also, if an existing call overlaps the region this much then the new variant will not '
                             'be created')
    parser.add_argument('--min-region-size', type=int, default=10000, help='Min region/subdivision size for revisions')
    parser.add_argument('--max-new-variant-size', type=float, default=2.0,
                        help='Max allowable size for new variants, as a multiple of the region size')
...
def main(argv: Optional[List[Text]] = None):
    ...
    genotypes = read_geno_file(list_path=args.geno_file_list, del_ids=gdr_del_ids, dup_ids=gdr_dup_ids,
    ...
    with tempfile.NamedTemporaryFile(dir=args.temp, suffix=".vcf.gz") as temp_vcf:
        revise_genotypes_and_create_records(input_vcf_path=args.vcf,
                                            ...
                                            max_new_variant_size=args.max_new_variant_size,
                                            ploidy_table_dict=ploidy_table_dict,
                                            batch=args.batch)
