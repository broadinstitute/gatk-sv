...
filterMetricStats <- function(dat, metric, min.metric=0){
  #Get variant counts
  max.inh <- nrow(dat[which(dat$label=="inh"),])
  ret.inh <- nrow(dat[which(dat[, paste("pro", metric, sep="_")] > min.metric & dat$label=="inh"),])
...
getMetricStats <- function(dat, trio, metric, metric.range){
  ...
  iter.res <- as.data.frame(t(sapply(metric.range, function(min.metric){
    stats <- filterMetricStats(trio_dat, metric, min.metric)
    return(stats)
  })))
