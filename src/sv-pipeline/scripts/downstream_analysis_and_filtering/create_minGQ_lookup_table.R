...
process.inputs <- function(CONDITIONS, STATS, ROC_OPTS, metric){
  #Read data
  ...
  colnames(opts) <- c("condition", paste("min", metric, sep="_"), "inh.ret", 
                      "inh.ret.rate", "dn.ret", "dn.rate")
  ...
  numeric.cols.idx <- which(colnames(merged) %in% c("minSVLEN", "maxSVLEN", "minAF", "maxAF",
                                                    ...
                                                    paste("min", metric, sep="_"), "inh.ret", 
                                                    "inh.ret.rate", "dn.ret", "dn.rate"))
...
prune.tree <- function(dat, order.tree, metric){
  cutoff.name <- paste("min", metric, sep="_")
  #Simplify table
  res <- dat[which(dat$n.filt==max(dat$n.filt)),
             which(colnames(dat) %in% c("condition", "minSVLEN", "maxSVLEN", 
                                        "minAF","maxAF", "includeSVTYPE",
                                        "excludeSVTYPE", "includeFILTER", 
                                        "excludeFILTER", "includeEV",
                                        "excludeEV", cutoff.name))]
  res$source <- NA
  #Update metric cutoff
  for(i in 1:nrow(res)){
    #If cutoff is already set, leave as is & update source
    if(!is.na(res[i, cutoff.name])){
      res$source[i] <- "ROC"
      
      #If cutoff is not already set, determine closest parent node that is set
    }else{
      ...
      #Walk up the branch until cutoff is set
      ...
      n.cutoff <- dat[next.branch, cutoff.name]
