...
build_marginal_APS_model.by_all_SVs<-function(dat_2, func_anno, aps_model_prefix, output_file_name){
  ...
  print('split intragenic SVs as testing data ... ')
  ...
  print('add support evidences for training data ...')
  ...
  print('add support evidences for testing data ...')
  ...
  print('correct the size grids for testing data according to training data ... ')
  ...
  print('build the marginal APS model ... ')
...
reorganize_supp_evi.all_sizes<-function(pass){
  pass$EVIDENCE = as.character(pass$EVIDENCE)
  pass$genomic_context = as.character(pass$genomic_context)
...
if(length(colnames(dat)[colnames(dat)=="FILTER"])>0){
	print('restrict to PASS events ...')
        dat = dat[dat$FILTER=="PASS",]
}
print('restrict to autosomal PASS events ...')
dat_2 = dat[!is.na(dat$AF) & !dat$X.chrom%in%c('chrX','chrY') & !is.na(dat$SVLEN),]
...
print('calculate singleton proportion by svtype ... ')
sp_by_svtype = calcu_singleton_proportion.by_svtype(dat_2)
print('calculate singleton proportion by genomic context ...')
sp_by_gc = calcu_singleton_proportion.by_genomic_context(dat_2)
print('calculate singleton proportion by SV sizes ... ')
sp_by_size = calcu_singleton_proportion.by_size(dat_2)
print('calculate singleton proportion by alignment evidences ... ')
