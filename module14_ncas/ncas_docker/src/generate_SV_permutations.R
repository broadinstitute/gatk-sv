...
trans$SVLEN = as.integer(trans$SVLEN)
trans[is.na(trans$SVLEN),]$SVLEN = 0
trans = trans[trans$SVLEN<5000000,]
while(TRUE){
	print(c(nrow(trans),nrow(trans_permu)))
        if(nrow(trans)> permu_set){
                ...
                trans = trans[!trans$name%in%out$name,]    
        }
        if(nrow(trans)<=permu_set){
                out = trans
                chrom = chr_arms[sample(c(1:nrow(chr_arms)),1),]
...
trans_permu = trans_permu[!is.na(trans_permu[,1]),]
