...
workflow IdentifyOutlierSamples {
  ...
  if (!defined(sv_counts)) {
    ...
    call CombineCounts as CombineShard {
      input:
  ...
  File final_counts = select_first([sv_counts, CombineShard.summed_svcounts])
...
# Combine SV count files across vcf shards
task CombineCounts {
  input{
    Array[File] svcounts
    String prefix
  ...
  command <<<
    ...
    for file in ~{sep=" " svcounts}; do
      cat "$file" | sed '1d' >> combined_SV_counts.tsv
    done

    python3 <<CODE
    fin=open("combined_SV_counts.tsv")
    out_hash={}
    for line in fin:
      pin=line.strip().split()
      if not pin[0] in out_hash.keys():
        out_hash[pin[0]] = {}
      if not pin[1] in out_hash[pin[0]].keys():
        out_hash[pin[0]][pin[1]]=0
      out_hash[pin[0]][pin[1]]+=int(pin[2])
    fin.close()

    fo=open("~{prefix}.summed_svcounts_per_sample.txt",'w')
    print('\t'.join(['sample','svtype','count']), file=fo)
    for i in out_hash.keys():
      if not i=='sample':
        for j in out_hash[i].keys():
          print('\t'.join([i,j, str(out_hash[i][j])]), file=fo)
    fo.close()
