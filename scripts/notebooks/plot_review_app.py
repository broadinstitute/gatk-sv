...
"""
Plot Review Application for Genomic Disorder Analysis

Usage examples:

1. Basic usage - Create new review session:
   app = PlotReviewApp(plot_directory='./plots', 
                       manifest_file='./manifest.json',
                       gd_regions_df=gd_regions)
   app.display()

2. Import previous results from summary table:
   # First, convert summary table to manifest
   create_manifest_from_summary_table(
       summary_table_file='./previous_results.tsv',
       plot_directory='./plots',
       output_manifest_file='./review_manifest.json'
   )
   
   # For debugging unmatched samples, use verbose=True
   create_manifest_from_summary_table(
       summary_table_file='./previous_results.tsv',
       plot_directory='./plots',
       output_manifest_file='./review_manifest.json',
   ...
   # Then load the app with this manifest for review
   app = PlotReviewApp(plot_directory='./plots',
                       manifest_file='./review_manifest.json',
                       gd_regions_df=gd_regions)
   app.display()

3. Generate summary table from manifest:
   summary_df = generate_summary_table(manifest_file='./manifest.json',
                                       gd_regions_df=gd_regions)
...
def create_manifest_from_summary_table(summary_table_file, plot_directory, output_manifest_file, verbose=False):
    """
    Convert a summary table (generated by generate_summary_table) into a manifest file.
    Carriers are marked as 'correct' and non-carriers as 'incorrect'.
    
    Args:
        summary_table_file: Path to the summary table file (CSV or TSV)
        plot_directory: Directory containing the plot files
        output_manifest_file: Path where the manifest JSON will be saved
        verbose: If True, print detailed debugging information for unmatched samples
    
    Returns:
        dict: The created manifest
    """
    # Detect delimiter and load summary table
    if not os.path.exists(summary_table_file):
        raise FileNotFoundError(f"Summary table file not found: {summary_table_file}")
    
    with open(summary_table_file, 'r') as f:
        first_line = f.readline()
        delimiter = '\t' if '\t' in first_line else ','
    
    summary_df = pd.read_csv(summary_table_file, sep=delimiter)
    
    # Validate required columns
    required_cols = ['chr', 'start', 'end', 'GD_ID', 'SVTYPE', 'carriers', 'non_carriers']
    missing_cols = [col for col in required_cols if col not in summary_df.columns]
    if missing_cols:
        raise ValueError(f"Summary table missing required columns: {missing_cols}")
    
    # Get all plot files
    plot_dir = Path(plot_directory)
    if not plot_dir.exists():
        raise FileNotFoundError(f"Plot directory not found: {plot_directory}")
    
    extensions = ['.jpg', '.jpeg', '.png']
    plot_files = []
    for ext in extensions:
        plot_files.extend(plot_dir.glob(f'*{ext}'))
    plot_files = [str(f) for f in plot_files]
    
    # Parse all plot files to create a lookup
    plot_lookup = {}  # key: (chr, start, end, gd_id, svtype, sample_id) -> filepath
    plot_lookup_by_sample = {}  # key: sample_id -> list of (chr, start, end, gd_id, svtype, filepath)
    ...
    for plot_file in plot_files:
        ...
        if plot_info:
            ...
            plot_lookup[key] = plot_file
            
            # Also index by sample_id for debugging
            sample_id = plot_info['sample_id']
            if sample_id not in plot_lookup_by_sample:
                plot_lookup_by_sample[sample_id] = []
            plot_lookup_by_sample[sample_id].append({
                'chr': plot_info['chr'],
                'start': plot_info['start'],
                'end': plot_info['end'],
                'gd_id': plot_info['gd_id'],
                'svtype': plot_info['svtype'],
                'filepath': plot_file
            })
    ...
    matched_count = 0
    unmatched_samples = []
    unmatched_details = []  # Store detailed info for verbose output
    ...
    for _, row in summary_df.iterrows():
        ...
        if gd_id == "GD_10q11.22-q11.23-AD_DEL_chr10_46005406_49845537":
            print(f"DEBUG: Processing GD_10q11.22-q11.23-AD_DEL_chr10_46005406_49845537 region {chr_val}:{start}-{end} SVTYPE:{svtype}")
        
        # Process carriers (mark as correct)
        carriers_str = str(row['carriers']).strip()
        if carriers_str and carriers_str != 'nan':
            carriers = [s.strip() for s in carriers_str.split(',') if s.strip()]
            for sample_id in carriers:
                if sample_id == "__sp0148883__35d430":
                    print(f"DEBUG: Processing non-carrier sample {sample_id} for region {chr_val}:{start}-{end} GD_ID:{gd_id} SVTYPE:{svtype}")
                key = (gd_id, sample_id)
                if key in plot_lookup:
                    plot_file = plot_lookup[key]
                    manifest[plot_file] = {
                        'classification': 'correct',
                        'subtype': 'typical',  # Default subtype
                        'notes': 'Auto-imported from summary table as carrier',
                    ...
                    unmatched_samples.append(f"{chr_val}:{start}-{end}_{gd_id}_{svtype}_{sample_id}")
                    if verbose:
                        ...
                        diagnostic = {
                            ...
                            'svtype': svtype,
                            'sample_id': sample_id,
                            'sample_plots': plot_lookup_by_sample.get(sample_id, [])
                        }
                        unmatched_details.append(diagnostic)
        
        # Process non-carriers (mark as incorrect)
        non_carriers_str = str(row['non_carriers']).strip()
        if non_carriers_str and non_carriers_str != 'nan':
            non_carriers = [s.strip() for s in non_carriers_str.split(',') if s.strip()]
            for sample_id in non_carriers:
                if sample_id == "__sp0148883__35d430":
                    print(f"DEBUG: Processing non-carrier sample {sample_id} for region {chr_val}:{start}-{end} GD_ID:{gd_id} SVTYPE:{svtype}")
                key = (gd_id, sample_id)
                if key in plot_lookup:
                    ...
                    manifest[plot_file] = {
                        ...
                        'notes': 'Auto-imported from summary table as non-carrier',
                        'timestamp': datetime.now().isoformat(),
                        'filename': plot_file,
                        'chr': chr_val,
                        'start': start,
    ...
    # Print summary
    print(f"Manifest created: {output_manifest_file}")
    print(f"Total plot files found: {len(plot_files)}")
    print(f"Matched and classified: {matched_count}")
    print(f"Unmatched samples: {len(unmatched_samples)}")
    
    # Print verbose diagnostics if requested
    if verbose and unmatched_details:
        print("\n" + "="*80)
        print("DETAILED DIAGNOSTICS FOR UNMATCHED SAMPLES")
        print("="*80)
        for detail in unmatched_details[:10]:  # Show first 10 in detail
            print(f"\nUNMATCHED {detail['type'].upper()}:")
            print(f"  Sample ID: {detail['sample_id']}")
            print(f"  Expected chr: {detail['chr']}")
            print(f"  Expected start: {detail['start']}")
            print(f"  Expected end: {detail['end']}")
            print(f"  Expected GD_ID: {detail['gd_id']}")
            print(f"  Expected svtype: {detail['svtype']}")
            
            if detail['sample_plots']:
                print(f"  Found {len(detail['sample_plots'])} plot(s) for this sample:")
                for plot in detail['sample_plots']:
                    print(f"    - chr:{plot['chr']} start:{plot['start']} end:{plot['end']}")
                    print(f"      GD_ID:{plot['gd_id']}")
                    print(f"      svtype:{plot['svtype']}")
                    print(f"      file:{os.path.basename(plot['filepath'])}")
                    # Show what didn't match
                    mismatches = []
                    if str(plot['chr']) != str(detail['chr']):
                        mismatches.append(f"chr({plot['chr']}≠{detail['chr']})")
                    if plot['start'] != detail['start']:
                        mismatches.append(f"start({plot['start']}≠{detail['start']})")
                    if plot['end'] != detail['end']:
                        mismatches.append(f"end({plot['end']}≠{detail['end']})")
                    if plot['gd_id'] != detail['gd_id']:
                        mismatches.append(f"gd_id({plot['gd_id']}≠{detail['gd_id']})")
                    if plot['svtype'] != detail['svtype']:
                        mismatches.append(f"svtype({plot['svtype']}≠{detail['svtype']})")
                    if mismatches:
                        print(f"      MISMATCH: {', '.join(mismatches)}")
            else:
                print(f"  No plots found for sample ID: {detail['sample_id']}")
        
        if len(unmatched_details) > 10:
            print(f"\n... and {len(unmatched_details) - 10} more unmatched samples")
    elif unmatched_samples and len(unmatched_samples) <= 20:
        print("\nUnmatched samples:")
        for sample in unmatched_samples:
            print(f"  - {sample}")
    elif unmatched_samples:
        print(f"\n(Showing first 20 of {len(unmatched_samples)} unmatched samples)")
...
class PlotReviewApp:
        ...
        if classification_filter != "All":
            ...
            for plot_file in filtered_files:
                ...
                if classification_filter == "Correct" and classification == "correct":
                    ...
                    temp_files.append(plot_file)
                elif classification_filter == "Unsure" and classification == "unsure":
                    temp_files.append(plot_file)
            filtered_files = temp_files
    ...
    def setup_ui(self):
        ...
        self.classification_filter_dropdown = widgets.Dropdown(
            options=["All", "Correct", "Incorrect", "Unsure"],
            value="All",
    ...
    def update_display(self):
        ...
        if not self.plot_files:
            ...
            # Just reset the toggle button states without modifying filters
            # This prevents the infinite loop of resetting filters
