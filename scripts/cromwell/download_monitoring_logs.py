...
def download(data, output_dir):
  ...
  if job_id != 'na':
    output_dest = output_dir + '/' + alias + '.' + str(shard_index) + '.' + str(attempt) + '.' + job_id + '.monitoring.log'
...
def main():
  ...
  Parallel(n_jobs=NUM_THREADS)(delayed(download)(d, output_dir) for d in call_metadata)
