...
SCRIPT_CONFIGS = {
  "GATKSVPipelinePhase1" : ScriptConfig(GATKSVPIPELINEPHASE1_MAP,
                                        sample_ids_keys = ("Module00a","inputs","samples"),
  ...
  "Module04" : ScriptConfig(MODULE04_MAP) # No sample order checking post-exclusion
}
...
def determine_workflow_name(default_inputs):
  ...
  if not workflow_name:
    raise_error('Workflow name could not be determined from the WDL input file')
  return workflow_name

def get_workflow_config(workflow_name):
  if workflow_name not in SCRIPT_CONFIGS:
    raise_error('Could not find workflow "' + workflow_name + '", options are: ' + str(SCRIPT_CONFIGS.keys()))
...
def check_all_metadata_present(script_config, metadata_files):
  if script_config.data_map.keys() != metadata_files.keys():
    raise_error('Script config workflows and metadata file workflows did not match. Script config expected ' + str(workflows) + ' but got metadata for ' + str(metadata_files.keys()))
  ...
  for workflow in script_config.data_map:
    if "outputs" in script_config.data_map[workflow]:
      for output_name in script_config.data_map[workflow]["outputs"]:
        wdl_input_name = workflow_name + "." + script_config.data_map[workflow]["outputs"][output_name]
        if wdl_input_name not in default_inputs:
          raise_error('Script configuration expected field ' + wdl_input_name + ' but it was not found in the WDL input file')
...
def load_prerequisite_metadata(metadata_files):
  ...
  for workflow in metadata_files:
    with open(metadata_files[workflow], 'r') as f:
      m = json.load(f)
      if 'outputs' not in m:
        raise_error('Metadata ' + metadata_files[workflow] + ' did not have an outputs field')
...
def get_preqreq_values(workflow_map, workflow_metadata, script_config, prereq_attr_prefix, workflow_name, inputs):
  for expected_name in workflow_map:
    ...
    if name not in workflow_metadata or not workflow_metadata[name]:
      print_warning('could not find metadata for attribute ' + name + ', using default value if provided\n')
    else:
...
def get_workflow_inputs(prereq_metadata, script_config, default_inputs, workflow_name):
  ...
  for prereq_workflow_name in script_config.data_map:
    ...
    if "inputs" in data_maps:
      get_preqreq_values(data_maps["inputs"], workflow_metadata["inputs"], script_config, "", workflow_name, inputs)
    if "outputs" in data_maps:
      get_preqreq_values(data_maps["outputs"], workflow_metadata["outputs"], script_config, prereq_workflow_name + ".", workflow_name, inputs)
...
def get_samples_list(script_config, prereq_metadata):
  ...
  if samples_workflow_metadata_key not in prereq_metadata[samples_workflow]:
    raise_error("Expected to find key " + samples_workflow_metadata_key + " in workflow " + samples_workflow + " metadata, but found: " + str(prereq_metadata[samples_workflow].keys()))
  if samples_attr not in prereq_metadata[samples_workflow][samples_workflow_metadata_key]:
    raise_error("Expected to find attribute " + samples_workflow_metadata_key + " : { " + samples_attr + " } in workflow " + samples_workflow)
...
def cross_check_sample_order(workflow_name, script_config, inputs, samples_list):
  for sample_specific_name in [workflow_name + "." + name for name in script_config.sample_specific_file_lists]:
    if sample_specific_name not in inputs:
      raise_error('Expected to find sample-specific parameter list ' + sample_specific_name)
    sample_specific_values = inputs[sample_specific_name]
    if not isinstance(sample_specific_values, list):
      raise_error('Expected sample-specific value ' + sample_specific_name + ' to be of type list but found ' + str(type(sample_specific_values)))
    if len(sample_specific_values) != len(samples_list):
      print_warning('Length of samples list is ' + str(len(samples_list)) + ' but length of sample-specific parameter ' + sample_specific_name + ' was ' + str(len(sample_specific_values)))
    for i in range(len(samples_list)):
      sample_id = samples_list[i]
      if sample_id not in sample_specific_values[i]:
        print_warning('Did not find sample id ' + sample_id + ' in input ' + sample_specific_name + '[' + str(i) + '], found ' + str(sample_specific_values[i]))

# Main function
def main():
  parser = argparse.ArgumentParser()
  parser.add_argument("default_inputs", help="Inputs JSON file containing default parameter values")
  parser.add_argument("prereq_workflow_paths", help="JSON file specifying metadata file paths for each prerequisite workflow with format \"workflow_name\" : \"/path/to/metadata\"")
  ...
  inputs = get_workflow_inputs(prereq_metadata, script_config, default_inputs, workflow_name)
  ...
  if script_config.requires_sample_ids():
    ...
    if script_config.sample_specific_file_lists:
      cross_check_sample_order(workflow_name, script_config, inputs, samples_list)
