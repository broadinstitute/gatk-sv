...
def estimate_costs_per_task(data):
  ...
  data['OptCPUCost'] = np.multiply(np.fmax(data['MaxCPU'], MIN_CPU), data['Hours']) * COST_CPU_HR
...
def estimate_costs_per_group(data):
  ...
  for group in groups:
    ...
    max_cpu = np.nan if np.isnan(d['MaxCPU']).all() else np.max(d['MaxCPU'])
    ...
    group_data['DynCost'] = sum((group_data['DynCPUCost'], group_data['DynMemCost'], group_data['DynDiskCost']))
...
def main():
  ...
  parser.add_argument("log_summary_file", help="Path to log summary TSV from get_cromwell_resource_usage2.sh -u -r")
