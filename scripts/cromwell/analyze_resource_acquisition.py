...
def check_workflow_valid(metadata, metadata_file, override_warning):
  ...
  if 'status' not in metadata:
    raise RuntimeError("Incomplete metadata input file %s. File lacks workflow status field." % metadata_file)
  ...
  if metadata['status'] == "Failed":
    logging.warning("Workflow failed, which is likely to impact plot accuracy.")
    found_retryable_error = True
  for event in metadata['workflowProcessingEvents']:
    if event['description'] == "Released":
      logging.warning("Server was interrupted during workflow execution, which is likely to impact plot accuracy.")
      found_retryable_error = True
      break
...
def check_file_nonempty(f):
  if not isfile(f):
    raise RuntimeError("Required metadata input file %s does not exist." % f)
  elif getsize(f) == 0:
