...
def write_data(data, file_path, header):
  with open(file_path,  'w') as f:
    ...
    for key in data.index:
      f.write(key + '\t' + '\t'.join([str(x) for x in data.loc(key)]) + '\n')
    ...
    with open(filepath, 'r') as f:
      ...
      for line in f:
        ...
        if start_time is None and line.startswith('['):
          start_time = datetime.strptime(line.strip()[1:-1], TIME_FORMAT)
        if line.startswith('['):
          end_time = datetime.strptime(line.strip()[1:-1], TIME_FORMAT)
      ...
      if len(mem_gb_data_f) > 0 and len(disk_gb_data_f) > 0:
        ...
        delta_hour = (delta_time.total_seconds() / 3600.) + (overhead_min / 60.0)
        ...
        cost_mem_opt = COST_PER_GB_MEM_HR * max(max_mem_gb, MIN_MEM_GB) * delta_hour

        cost_disk = COST_PER_GB_DISK_HR * (total_disk + BOOT_DISK_GB) * delta_hour
        cost_disk_opt = COST_PER_GB_DISK_HR * (max(max_disk_gb, MIN_DISK_GB) + BOOT_DISK_GB) * delta_hour

        cost_cpu = COST_CPU_HR * total_cpu * delta_hour
        cost_cpu_opt = COST_CPU_HR * max(max_cpu, MIN_MEM_GB) * delta_hour
...
def calc_group(data):
  ...
  for task in task_names:
    ...
    cost_disk_static = COST_PER_GB_DISK_HR * (max(max_disk, MIN_DISK_GB) + BOOT_DISK_GB) * hours
...
def do_simple_bar(data, xticks, path, bar_width=0.35, height=12, width=12,
  ...
  plt.figure(num=None, figsize=(width, height), dpi=100, facecolor='w', edgecolor='k')
...
def main():
  ...
  parser.add_argument("log_dir", help="Path containing monitoring script logs ending in \".monitoring.log\"")
  ...
  create_graphs(group_df, out_file, semilog=args.semilog, num_samples=plot_norm)
