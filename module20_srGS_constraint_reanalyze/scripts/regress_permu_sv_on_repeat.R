...
#
# For each SV feature column (default: 37+) in permutation TSV files:
#   1. Pool data across permutations 1-100
#      - For each gene x permutation: sv_count, SD_overlap, SR_overlap
#   2. Fit: sv_count ~ SD_overlap + SR_overlap  (lm)
#   3. Predict expected sv_count for permutation 0 using SD[col="0"] and SR[col="0"]
#   4. Compare predicted vs observed (from seed0 file)
#
# Outputs:
#   sv_predicted_seed0.tsv.gz   -- gene x SV_feature predicted values for seed 0
#   sv_observed_seed0.tsv.gz    -- gene x SV_feature observed values from seed 0
#   regression_summary.tsv      -- per SV feature: coefficients, R², N
