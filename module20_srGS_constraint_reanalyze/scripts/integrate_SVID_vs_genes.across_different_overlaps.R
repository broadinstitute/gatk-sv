...
option_list = list(
        ...
        make_option(c('-a', "--appendix"), type="character", default=NULL, help="appendix inserted before .reorganized in input file names", metavar="character"),
...
d1a=read.table(paste(opt$prefix, '.inside_exons.',              opt$appendix, '.reorganized', sep=''), header=T)
colnames(d1a)[4] = 'inside_exons'
d1b=read.table(paste(opt$prefix, '.whole_transcript_overlap.',  opt$appendix, '.reorganized', sep=''), header=T)
colnames(d1b)[4] = 'whole_transcript_overlap'
d1=merge(d1a[,c(1,2,4)], d1b[,c(1,2,4)], by=c('SVID','svtype'), all=T)
print(dim(d1))

d2=read.table(paste(opt$prefix, '.intact_exon_overlap.',       opt$appendix, '.reorganized', sep=''), header=T)
colnames(d2)[4] = 'intact_exon_overlap'
dat=merge(d1, d2[,c(1,2,4)], by=c('SVID','svtype'), all=T)

d3=read.table(paste(opt$prefix, '.partial_exon_overlap.',      opt$appendix, '.reorganized', sep=''), header=T)
colnames(d3)[4] = 'partial_exon_overlap'
dat=merge(dat, d3[,c(1,2,4)], by=c('SVID','svtype'), all=T)

d4b=read.table(paste(opt$prefix, '.tss_transcripts_overlap.',   opt$appendix, '.reorganized', sep=''), header=T)
colnames(d4b)[4] = 'tss_transcripts_overlap'
dat=merge(dat, d4b[,c(1,2,4)], by=c('SVID','svtype'), all=T)

d5b=read.table(paste(opt$prefix, '.partial_transcripts_overlap.', opt$appendix, '.reorganized', sep=''), header=T)
colnames(d5b)[4] = 'partial_transcripts_overlap'
dat=merge(dat, d5b[,c(1,2,4)], by=c('SVID','svtype'), all=T)

d6 = read.table(paste(opt$prefix, '.5_prime_utr.',              opt$appendix, '.reorganized', sep=''), header=T)
colnames(d6)[4] = 'X5_prime_utr'
dat=merge(dat, d6[,c(1,2,4)], by=c('SVID','svtype'), all=T)

d7 = read.table(paste(opt$prefix, '.3_prime_utr.',              opt$appendix, '.reorganized', sep=''), header=T)
colnames(d7)[4] = 'X3_prime_utr'
dat=merge(dat, d7[,c(1,2,4)], by=c('SVID','svtype'), all=T)

d8=read.table(paste(opt$prefix, '.inside_introns.',            opt$appendix, '.reorganized', sep=''), header=T)
