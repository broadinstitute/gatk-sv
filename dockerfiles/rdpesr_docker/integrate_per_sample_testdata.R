...
option_list = list(
        make_option(c( "--filename_list"), type="character", default=NULL, help="name of bed file ", metavar="character"),
...
readin_sample_data<-function(filename_list, i){
  gq_recali = read.table(as.character(filename_list[i,]$GQ_reacali), header=T)
  print(as.character(filename_list[i,]$GQ_reacali))

  boost_dyna = read.table(as.character(filename_list[i,]$boost_dyna), header=T)
  print(as.character(filename_list[i,]$boost_dyna))

  boost_fix = read.table(as.character(filename_list[i,]$boost_fix), header=T)
  print(as.character(filename_list[i,]$boost_fix))

  #minGQ_1 = read.table(as.character(filename_list[i,]$minGQ1perc), header=T)
  #print(as.character(filename_list[i,]$minGQ1perc))

  #minGQ_5 = read.table(as.character(filename_list[i,]$minGQ5perc), header=T)
  #print(as.character(filename_list[i,]$minGQ5perc))

  minGQ_10 = read.table(as.character(filename_list[i,]$minGQ10perc), header=T)
  print(as.character(filename_list[i,]$minGQ10perc))


  overall_gt = readin_cleanvcf(as.character(filename_list[i,]$loose_union))
  ...
  bs=read.table(as.character(filename_list[i,]$boost_score), header=T)
  dat=merge(dat, bs, by='SVID')
  print(nrow(dat))
...
filename_list=read.table(opt$filename_list, header=T)
...
for(i in 1:nrow(filename_list)){
  ...
  dat = readin_sample_data(filename_list,i)
  ...
  write.table(out_polish, as.character(filename_list[i,]$output), quote=F, sep='\t', col.names=T, row.names=F)
}
