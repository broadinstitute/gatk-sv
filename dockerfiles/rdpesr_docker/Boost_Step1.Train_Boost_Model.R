#!/usr/bin/env Rscript
#script to train the boost model
...
option_list = list(
  ...
  make_option(c( "--qc_plots"), type="character", default=NULL, help="path to keep the qc plots from cross validation", metavar="character"),
  make_option(c( "--sv_colors"), type="character", default=NULL, help="2 column file with svtype and color", metavar="character")
);
...
qc_path = opt$qc_plots
sv_colors = opt$sv_colors

train_site_path = paste(paste(strsplit(train_site_path,'/')[[1]],collapse = '/'),'/',sep = '')
train_sample_path = paste(paste(strsplit(train_sample_path,'/')[[1]],collapse = '/'),'/',sep = '')
train_LD_path = paste(paste(strsplit(train_LD_path,'/')[[1]],collapse = '/'),'/',sep = '')
train_lg_cnv = paste(paste(strsplit(train_lg_cnv,'/')[[1]],collapse = '/'),'/',sep = '')
train_model_path = paste(paste(strsplit(train_model_path,'/')[[1]],collapse = '/'),'/',sep = '')
qc_path = paste(paste(strsplit(qc_path,'/')[[1]],collapse = '/'),'/',sep = '')


cbPalette <- c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")
col_table=read.table(sv_colors, comment.char = "")
...
sample_list = unique(training_data$sample)
fold_num = 5
test_num = round(length(sample_list)/fold_num)+1
for(i in c(1:fold_num)){
  testing_samp_list = sample_list[((i-1)*test_num+1) : (i*test_num)]
  training_samp_list = sample_list[!sample_list%in%testing_samp_list]
  cv_training_data = training_data[ training_data$sample%in%training_samp_list,]
  cv_testing_data  = training_data[ training_data$sample%in%testing_samp_list,]

  cv_bgm_models = train_del_dup_ins(cv_training_data,feature_list_cnv, feature_list_oth, LD_SVID, ssc_hq, ssc_lq)
  cv_bgm_tests = test_del_dup_ins(cv_bgm_models,cv_testing_data,feature_list_cnv, feature_list_oth, -1)
  write.table(cv_bgm_tests[,c("SVID","sample","preds_builtin","preds_out")], paste('trainin_sets/cross_validation/cv_round',i,'tsv', sep='.'), quote=F, sep='\t', col.names=T, row.names=F)
  }

# QC CV results:
r1=read.table(paste(train_model_path, 'cross_validation_results.round.1.tsv', sep = ''), header = T)
r2=read.table(paste(train_model_path, 'cross_validation_results.round.2.tsv', sep = ''), header = T)
r3=read.table(paste(train_model_path, 'cross_validation_results.round.3.tsv', sep = ''), header = T)
r4=read.table(paste(train_model_path, 'cross_validation_results.round.4.tsv', sep = ''), header = T)
r5=read.table(paste(train_model_path, 'cross_validation_results.round.5.tsv', sep = ''), header = T)

pdf(paste(qc_path,'count_SVs_by_bs.CV_all_samples.pdf', sep = ''))
plot_SVCounts_by_bs_score_all_CV(training_data, r1,r2,r3,r4,r5)
dev.off()

pdf(paste(qc_path,'FDR_vs_bs_cff.CV_all.pdf', sep = ''))
plot_fdr_vs_BScff(training_data, r1,r2,r3,r4,r5)
dev.off()

for(cff in c(-2,-1,0)){
  print(cff)
  png(paste(qc_path,'upsetplot.CV_all_rounds.bs_',cff,'.png',sep=''))
  plot_upset_all_CV(training_data, r1,r2,r3,r4,r5, cff)
  dev.off()
  for(svtype in c('DEL','DUP','INS','INV')){
    print(svtype)
    png(paste(qc_path,'upsetplot.CV_all_rounds.bs_',cff,'.',svtype,'.png',sep=''))
    plot_upset_all_CV(training_data[training_data$SVTYPE.x==svtype,], r1,r2,r3,r4,r5, cff)
    dev.off()
  }
}

png(paste(qc_path,'upsetplot.CV_all_rounds.bs_-1.round.1.png',sep=''))
plot_upset_each_CV(training_data, r1, -1)
dev.off()
png(paste(qc_path,'upsetplot.CV_all_rounds.bs_-1.round.2.png',sep=''))
plot_upset_each_CV(training_data, r2, -1)
dev.off()
png(paste(qc_path,'upsetplot.CV_all_rounds.bs_-1.round.3.png',sep=''))
plot_upset_each_CV(training_data, r3, -1)
dev.off()
png(paste(qc_path,'upsetplot.CV_all_rounds.bs_-1.round.4.png',sep=''))
plot_upset_each_CV(training_data, r4, -1)
dev.off()
png(paste(qc_path,'upsetplot.CV_all_rounds.bs_-1.round.5.png',sep=''))
plot_upset_each_CV(training_data, r5, -1)
dev.off()


# train bgm model
bgm_models = train_del_dup_ins(training_data,feature_list_cnv, feature_list_oth, LD_SVID, ssc_hq, ssc_lq)
saveRDS.lgb.Booster(bgm_models[[1]],  file = paste(train_model_path, 'bgm_model.DEL_s1_under250bp.rds', sep = ''), raw=TRUE)
saveRDS.lgb.Booster(bgm_models[[2]],  file = paste(train_model_path, 'bgm_model.DUP_s1_under250bp.rds', sep = ''), raw=TRUE)
saveRDS.lgb.Booster(bgm_models[[3]],  file = paste(train_model_path, 'bgm_model.DEL_s2_250bpto1Kb.rds', sep = ''), raw=TRUE)
saveRDS.lgb.Booster(bgm_models[[4]],  file = paste(train_model_path, 'bgm_model.DUP_s2_250bpto1Kb.rds', sep = ''), raw=TRUE)
saveRDS.lgb.Booster(bgm_models[[5]],  file = paste(train_model_path, 'bgm_model.DEL_s3_1to5Kb.rds', sep = ''), raw=TRUE)
saveRDS.lgb.Booster(bgm_models[[6]],  file = paste(train_model_path, 'bgm_model.DUP_s3_1to5Kb.rds', sep = ''), raw=TRUE)
saveRDS.lgb.Booster(bgm_models[[7]],  file = paste(train_model_path, 'bgm_model.DEL_s4_5to50Kb.rds', sep = ''), raw=TRUE)
saveRDS.lgb.Booster(bgm_models[[8]],  file = paste(train_model_path, 'bgm_model.DUP_s4_5to50Kb.rds', sep = ''), raw=TRUE)
saveRDS.lgb.Booster(bgm_models[[9]],  file = paste(train_model_path, 'bgm_model.DEL_s5_over50Kb.rds', sep = ''), raw=TRUE)
saveRDS.lgb.Booster(bgm_models[[10]], file = paste(train_model_path, 'bgm_model.DUP_s5_over50Kb.rds', sep = ''), raw=TRUE)
saveRDS.lgb.Booster(bgm_models[[11]], file = paste(train_model_path, 'bgm_model.INS.rds', sep = ''), raw=TRUE)
saveRDS.lgb.Booster(bgm_models[[12]], file = paste(train_model_path, 'bgm_model.ALU.rds', sep = ''), raw=TRUE)
saveRDS.lgb.Booster(bgm_models[[13]], file = paste(train_model_path, 'bgm_model.MEI.rds', sep = ''), raw=TRUE)
