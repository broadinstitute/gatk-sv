...
colnames(dat)=c('#CHR','POS','END', 'SVID','SVTYPE','SVLEN','AF','sample')
...
if(!is.null(opt$rd)){
    ...
    dat=merge(dat, rd[,c(4,ncol(rd)-2,ncol(rd)-1,ncol(rd))], by='SVID', all=T)
}
...
if(!is.null(opt$rd_le)){
    ...
    dat=merge(dat, rd_le[,c(4,ncol(rd_le)-2,ncol(rd_le)-1,ncol(rd_le))], by='SVID', all=T)
}
...
if(!is.null(opt$rd_ri)){
    ...
    dat=merge(dat, rd_ri[,c(4,ncol(rd_ri)-2,ncol(rd_ri)-1,ncol(rd_ri))], by='SVID', all=T)
}
...
if(!is.null(opt$pesr)){
    ...
    dat=merge(dat, pesr[,c(4,ncol(pesr)-5,ncol(pesr)-4,ncol(pesr)-3,ncol(pesr)-2,ncol(pesr)-1,ncol(pesr))], by='SVID', all=T)
...
if(!is.null(opt$info)){
    ...
    dat=merge(dat, info[,c('SVID','ALGORITHMS','EVIDENCE','FILTER','BothSideSupp')], by='SVID', all=T)
}

if(!is.null(opt$gt)){
    gtgq=read.table(opt$gt, header =T)
    dat=merge(dat, gtgq, by='SVID', all=T)
}

if(!is.null(opt$denovo)){
    dnv = read.table(opt$denovo, header =T)
    dat=merge(dat, dnv, by='SVID', all=T)
}
...
if(!is.null(opt$raw_manta)){
    ...
    dat=merge(dat, vs_manta[,c(4,8,9)], by='SVID', all=T)
}
...
if(!is.null(opt$raw_wham)){
    ...
    dat=merge(dat, vs_wham[,c(4,8,9)], by='SVID', all=T)
}
...
if(!is.null(opt$raw_melt)){
    ...
    dat=merge(dat, vs_melt[,c(4,8,9)], by='SVID', all=T)
}
...
if(!is.null(opt$raw_depth)){
    ...
    dat=merge(dat, vs_depth[,c(4,8,9)], by='SVID', all=T)
}
...
if(!is.null(opt$vs_hgsv)){
    ...
    dat=merge(dat, vs_hgsv[,c(4,8,9)], by='SVID', all=T)
}
...
if(!is.null(opt$vs_pacbio)){
    ...
    dat=merge(dat, vs_pacbio[,c(4,8,9)], by='SVID', all=T)
}
...
if(!is.null(opt$vs_bionano)){
    ...
    dat=merge(dat, vs_bionano[,c(4,8,9)], by='SVID', all=T)
}
