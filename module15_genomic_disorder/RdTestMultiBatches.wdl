...
task RemoteTabixRdMetrics {
  ...
  command <<<
    ...
    paste -d ' ' <(sed -e "s/^/sed -e 's/^/GCS_OAUTH_TOKEN=`gcloud auth application-default print-access-token` \\\n/'    tabix -h -R /" batch_list | sed -e 's/$/ | bgzip -c >/') <(sed -e 's/\//\t/g' batch_list | awk '{print $NF}' | sed -e 's/^/rd_metrics_folder\//') > remote_tabix.sh
