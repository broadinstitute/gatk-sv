...
workflow AnnoILFeaturesPerSample{
    ...
    if(run_duphold){
        scatter ( contig in contigs ) {
            ...
            call mini_tasks.Bcf2Vcf as Bcf2VcfIL{
                input:
                    prefix = sample,
                    contig = contig,
            ...
            call mini_tasks.Bcf2Vcf as Bcf2VcfIL_le_flank{
                input:
                    prefix = sample,
                    contig = contig,
            ...
            call mini_tasks.Bcf2Vcf as Bcf2VcfIL_ri_flank{
                input:
                    prefix = sample,
                    contig = contig,
        ...
        call mini_tasks.ConcatVcfs as ConcatVcfsIL_le_flank{
            input:
                ...
                merge_sort=true,
                outfile_prefix="~{sample}.IL_le_flank",
                sv_base_mini_docker=sv_base_mini_docker,
                runtime_attr_override=runtime_attr_ConcatVcfs
        }
        ...
        call mini_tasks.ConcatVcfs as ConcatVcfsIL_ri_flank{
            input:
                ...
                merge_sort=true,
                outfile_prefix="~{sample}.IL_ri_flank",
                sv_base_mini_docker=sv_base_mini_docker,
                runtime_attr_override=runtime_attr_ConcatVcfs
        }
    }
...
task VaporValidation{
    ...
    command <<<
        cut -f1-4,7 ~{bed} | grep -v INS > ~{prefix}.vapor_run.bed
        cut -f1-4,7,8 ~{bed} | grep INS | sed -e 's/INS\t/INS_/' >> ~{prefix}.vapor_run.bed
