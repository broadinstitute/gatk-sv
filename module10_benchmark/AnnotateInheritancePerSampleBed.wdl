...
import "AnnotateInheritancePerSamplePerBed.wdl" as annotate_inheritance_per_sample_per_bed
...
workflow AnnotateInheritancePerSample{
    ...
    scatter (i in range(length(cleanBeds))) {
        call annotate_inheritance_per_sample_per_bed.AnnotateInheritancePerSamplePerBed as AnnotateInheritancePerSamplePerBed {
            input:
    ...
    call mini_tasks.ConcatBeds as concat_inheri{
        input:
            shard_bed_files = AnnotateInheritancePerSamplePerBed.inheritance,
            prefix = probands_id,
            sv_base_mini_docker = sv_base_mini_docker
    }
