...
def add_Num_SR_le(sr_index,info, flank_length=100):
    ...
    fin=os.popen(r'''tabix %s %s:%d-%d'''%(sr_index, info[0],int(info[1])-flank_length, int(info[1])+flank_length))
    tmp=[]
    for line in fin:
        pin=line.strip().split()
        tmp.append(pin)
...
def add_Num_SR_ri(sr_index,info, flank_length=100):
    #eg of info:    ['chr1', '137221', '137339', 'HOM', 'INS']
    fin=os.popen(r'''tabix %s %s:%d-%d'''%(sr_index, info[0],int(info[2])-flank_length, int(info[2])+flank_length))
    tmp=[]
    for line in fin:
        pin=line.strip().split()
        tmp.append(pin)
...
def add_Num_PE_le(pe_index, info, flank_length=300):
    fin=os.popen(r'''tabix %s %s:%d-%d'''%(pe_index, info[0],int(info[1])-2*flank_length, int(info[1])+flank_length))
    tmp=[]
    for line in fin:
        pin=line.strip().split()
        if 'INS' in pin[4] or pin[4] in ['INS','ALU','LINE1','SVA']:
            ...
            if pin[0]==pin[3]:
                if abs(int(pin[4])-int(pin[1]))>100*(int(info[2])-int(info[1])): continue
                else: tmp.append(pin)
    fin.close()
    #if len(tmp)==0:
    #    return 0
...
def add_Num_PE_ri(pe_index, info, flank_length=300):
    fin=os.popen(r'''tabix %s %s:%d-%d'''%(pe_index, info[0],int(info[2])-flank_length, int(info[2])+2*flank_length))
    tmp=[]
    for line in fin:
        pin=line.strip().split()
        if 'INS' in pin[4] or pin[4] in ['INS','ALU','LINE1','SVA']:
...
def main():
    ...
    parser.add_argument('PB_bed', type=str, help='name of input PacBio bed file')
    parser.add_argument('pe_file', type=str, help='name of pe files with index')
    parser.add_argument('sr_file', type=str, help='name of sr files with index')
    parser.add_argument('output', type=str, help='name of output files with index')
