def calcu_inheri_hash(vcf_file,fam_file):	
	...
	for record in fvcf:
		...
		for child in fam_info.keys():
			...
			trio_len = sum([1 if i in record.samples.keys() else 0 for i in trio])
			if trio_len==3:
				gt = [record.samples[i]['GT'] for i in trio]
				if  (None, None) in gt: continue
				if gt == [(0, 0), (0, 0), (0, 0)]:	continue
				else:
					if gt[1]==(0, 0) and not gt[0]==(0, 0) and not gt[2]==(0, 0):
						inheri_hash[child].append(['fa_pb',record.info['SVTYPE']])
					if gt[0]==(0, 0) and not gt[1]==(0, 0) and not gt[2]==(0, 0):
						inheri_hash[child].append(['mo_pb',record.info['SVTYPE']])
					if not gt[0]==(0, 0) and not gt[1]==(0, 0) and not gt[2]==(0, 0):
						inheri_hash[child].append(['fa_mo_pb',record.info['SVTYPE']])
					if gt[0]==(0, 0) and gt[1]==(0, 0) and not gt[2]==(0, 0):
						inheri_hash[child].append(['denovo',record.info['SVTYPE']])
	fvcf.close()
	return inheri_hash
...
def trio_info_readin(fam_file):
	...
	for line in fin:
		...
		if pin[2]=='0' and pin[3]=='0':	continue
...
def write_output_stat(fileout,inheri_stat):
	fo=open(fileout,'w')
	print('\t'.join(['sample', 'svtype', 'fa_mo_pb','fa_pb','mo_pb','denovo']), file=fo)
