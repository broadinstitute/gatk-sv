...
workflow AnnotateILFeatures{
    ...
    output{
    	File filtered_results = apply_boost_filter.boost_results
    }


task apply_boost_filter{
    input{
        File site_feature_folder
        File model_folder
        File input_folder
    ...
    output{
        File boost_results = "output.tar.gz"
    }
    ...
    command <<<
        ...
        Rscript /src/Step3.apply_boost_filter.R \
