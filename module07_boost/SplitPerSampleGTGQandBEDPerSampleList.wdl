...
workflow SplitPerSampleGTGQandBEDPerSampleList{
    ...
    Array[File] cleanBedList = select_first([vcf2bed.vcf_bed, cleanBeds])


    scatter(i in range(length(cleanBedList))){
...
task split_per_sample_gtgq {
  ...
  command <<<
    ...
    python  /src/split_gtgq_per_sample.py ~{sample_list} ~{vcf} ~{prefix}

  >>>

  output {
    Array[File] gtgq_file =  glob("per_sample_GTGQ/*~{prefix}")
  }
...
task split_per_sample_bed {
  ...
  command <<<
    ...
    python  /src/split_bed_per_sample.py ~{sample_list} ~{bed} ~{prefix}


  >>>

  output {
    Array[File] bed_file =  glob("per_sample_bed/*~{prefix}")
  }
