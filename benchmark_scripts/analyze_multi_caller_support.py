...
def create_violin_plots(df, output_dir, max_n_matches, svtypes_filter=None):
    ...
    if all_data_for_ylim:
        min_val = min(all_data_for_ylim)
        max_val = max(all_data_for_ylim)
        margin = (max_val - min_val) * 0.1
        common_ylim = (max(0, min_val - margin), min(100, max_val + margin))
    else:
        common_ylim = (0, 100)
    ...
    for n_idx, n_matches in enumerate(range(1, max_n_matches + 1)):
        ...
        if data_to_plot:
            ...
            # Style the subplot
            ax.set_ylabel(f'Support Rate (%) - {n_matches}+ Callers', fontsize=10)
            ax.set_title(f'{n_matches}+ Caller Support', fontsize=12, fontweight='bold')
    ...
    fig.suptitle(f'Multi-Caller Support Rates by Genomic Context{svtype_suffix}\n(PASS+MULTIALLELIC calls, 50% reciprocal overlap)', 
...
def create_heatmap_tables(df, output_dir, max_n_matches, svtypes_filter=None):
    ...
    def create_heatmap_for_regions(regions, title_suffix, filename_suffix):
        ...
        for n_idx, n_matches in enumerate(range(1, max_n_matches + 1)):
            ...
            if available_sizes and available_svtypes:
                ...
                sns.heatmap(pivot_data, 
                           ...
                           fmt='', 
                           cmap='YlOrRd', 
                           cbar_kws={'label': f'Support Rate (%) - {n_matches}+ Callers'},
                           vmin=0, 
                           vmax=100,
                           ax=ax,
                           square=True)  # Make cells square
                
                ax.set_title(f'{title_suffix} - {n_matches}+ Callers')
                ax.set_xlabel('Size Bucket (bp)')
        ...
        # Add overall title
        svtype_suffix = f" ({', '.join(svtypes_filter)})" if svtypes_filter else ""
        fig.suptitle(f'Multi-Caller Support Rates - {title_suffix}{svtype_suffix}\n(PASS+MULTIALLELIC calls, 50% reciprocal overlap)', 
                    fontsize=14, fontweight='bold')
